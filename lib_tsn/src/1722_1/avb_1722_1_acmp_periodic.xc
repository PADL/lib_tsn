...
extern avb_1722_1_acmp_inflight_command
    acmp_controller_inflight_commands[AVB_1722_1_MAX_INFLIGHT_COMMANDS];
extern avb_1722_1_acmp_inflight_command
    acmp_listener_inflight_commands[AVB_1722_1_MAX_INFLIGHT_COMMANDS];
 ...
 *
 * The connected_to param equal to 1 is equivalent to
 * listenerIsConnectedTo(command) in spec
 */
static unsigned acmp_listener_is_connected(int connected_to, client interface avb_interface avb) {
    ...
    if (stream_is_reserved) {
        if (acmp_listener_streams[unique_id].talker_guid.l ==
                acmp_listener_rcvd_cmd_resp.talker_guid.l &&
...
void avb_1722_1_acmp_controller_periodic(client interface ethernet_tx_if i_eth,
    ...
    case ACMP_CONTROLLER_WAITING: {
        ...
        // acmp_inflight_timeout_idx is a global index provided to
...
#ifdef AVB_1722_1_ACMP_DEBUG_INFLIGHT
            debug_printf(
                "ACMP Controller: Removed inflight %s with timed out retry - "
                "seq id: %d\n",
...
#ifdef AVB_1722_1_ACMP_DEBUG_INFLIGHT
            debug_printf(
                "ACMP Controller: Sent retry for timed out %s - seq id: %d\n",
                debug_acmp_message_s[acmp_controller_inflight_commands[i].command.message_type],
                acmp_controller_inflight_commands[i].original_sequence_id);
#endif
        }
...
#if AVB_ENABLE_1722_1
            avb_talker_on_listener_connect_failed(
                avb, my_guid, acmp_controller_cmd_resp.talker_unique_id,
    ...
    case ACMP_CONTROLLER_GET_TX_CONNECTION_RESPONSE: {
        ...
        unsafe {
            ...
            if (inflight) {
                debug_printf("ACMP Controller: Removed inflight %s with response %s - "
                             "seq id: %d\n",
...
void avb_1722_1_acmp_listener_periodic(client interface ethernet_tx_if i_eth,
    ...
    case ACMP_LISTENER_CONNECT_TX_RESPONSE: {
        unsafe {
            ...
            if (acmp_listener_valid_listener_unique()) {
                ...
                if (inflight != NULL) {
                    acmp_listener_rcvd_cmd_resp.sequence_id =
                        inflight->original_sequence_id; // FIXME: This is a bit
                                                        // messy

#ifdef AVB_1722_1_ACMP_DEBUG_INFLIGHT
                    debug_printf("ACMP Listener: Removed inflight CONNECT_TX_COMMAND "
                                 "with response %s - seq id: %d\n",
                                 debug_acmp_status_s[inflight->command.status],
                    ...
                    if (acmp_listener_rcvd_cmd_resp.flags & AVB_1722_1_ACMP_FLAGS_CLASS_B) {
                        ...
                        acmp_listener_store_fast_connect_info(
...
#if AVB_ENABLE_1722_1
                        acmp_listener_rcvd_cmd_resp.status = avb_listener_on_talker_connect(
                            avb, acmp_listener_rcvd_cmd_resp.listener_unique_id,
                            acmp_listener_rcvd_cmd_resp.talker_guid,
                ...
                if (acmp_listener_rcvd_cmd_resp.flags & AVB_1722_1_ACMP_FLAGS_FAST_CONNECT) {
                    // Ideally we would go into ACMP_LISTENER_WAITING here, but
                    // some Controllers do not register fast connect RX
                    // responses as an active connection. So we force a Get RX
                    // State to notify the Controller that a connection was
                    // made.
...
#ifdef AVB_1722_1_ACMP_DEBUG_INFLIGHT
                    debug_printf("ACMP Listener: Removed inflight %d "
                                 "DISCONNECT_TX_COMMAND with response %s - seq id: %d\n",
                                 (int)inflight, debug_acmp_status_s[inflight->command.status],
                                 inflight->command.sequence_id);
#endif
                }
    ...
    case ACMP_LISTENER_GET_STATE: {
        ...
        if (!acmp_listener_valid_listener_unique()) {
            ...
            /* Sets appropriate state fields in acmp_listener_rcvd_cmd_resp for
             * responding and returns status message */
...
#ifdef AVB_1722_1_ACMP_DEBUG_INFLIGHT
            debug_printf("ACMP Listener: Removed inflight %d %s with timed out "
                         "retry - seq id: %d\n",
                         (int)inflight, debug_acmp_message_s[inflight->command.message_type],
                         inflight->command.sequence_id);
#endif
        } else {

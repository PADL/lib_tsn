...
#define GUID_NUM_BYTES 8

typedef union {
    uint64_t l;
    uint8_t c[GUID_NUM_BYTES];
...
#define GET_1722_1_DATALENGTH(pkt)                                                                 \
    ((((pkt)->valid_time_data_length_hi & 0x7) << 8) + ((pkt)->data_length_lo))
...
#define SET_1722_1_SV(pkt, val)      SET_BITS(&pkt->sv_avb_version_msg_type, 7, 7, val)

#define SET_1722_1_AVB_VERSION(pkt, val) SET_BITS(&pkt->sv_avb_version_msg_type, 4, 6, val)

#define SET_1722_1_MSG_TYPE(pkt, val) SET_BITS(&pkt->sv_avb_version_msg_type, 0, 3, val)

#define SET_1722_1_VALID_TIME(pkt, val) SET_BITS(&pkt->valid_time_data_length_hi, 3, 7, val)
...
#define GET_WORD(data, member)                                                                     \
    do {                                                                                           \
        data = (member[0] << 24) + (member[1] << 16) + (member[2] << 8) + (member[3] << 0);        \
    } while (0);
...
#define COMPARE_WORD(member, data) ((member[0] == (data & 0xffff)) && (member[1] == (data >> 16)))

...
static const uint8_t avb_1722_1_acmp_dest_addr[MACADDR_NUM_BYTES] = AVB_1722_1_ACMP_DEST_MAC;

// ACMP command timeouts (in centiseconds) as defined in Table 7.4 of the
// specification
...
avb_1722_1_acmp_inflight_command
    acmp_controller_inflight_commands[AVB_1722_1_MAX_INFLIGHT_COMMANDS];
...
void avb_1722_1_acmp_controller_init() {
    ...
    memset(acmp_controller_inflight_commands, 0,
           sizeof(avb_1722_1_acmp_inflight_command) * AVB_1722_1_MAX_INFLIGHT_COMMANDS);
...
void avb_1722_1_acmp_controller_deinit() { acmp_controller_state = ACMP_CONTROLLER_IDLE; }

void avb_1722_1_acmp_talker_init() { acmp_talker_state = ACMP_TALKER_WAITING; }

void avb_1722_1_acmp_listener_init() {
    int i;
    acmp_listener_state = ACMP_LISTENER_WAITING;
    memset(acmp_listener_inflight_commands, 0,
           sizeof(avb_1722_1_acmp_inflight_command) * AVB_1722_1_MAX_INFLIGHT_COMMANDS);

    for (i = 0; i < AVB_1722_1_MAX_LISTENERS; i++)
        acmp_zero_listener_stream_info(i);
...
/**
 * Creates a valid ACMP Ethernet packet in avb_1722_1_buf[] from a supplied
 * command structure
 */
void avb_1722_1_create_acmp_packet(avb_1722_1_acmp_cmd_resp *cr, int message_type, int status) {
    ...
    avb_1722_1_acmp_packet_t *pkt =
        (avb_1722_1_acmp_packet_t *)(hdr + AVB_1722_1_PACKET_BODY_POINTER_OFFSET);

    avb_1722_1_create_1722_1_header(avb_1722_1_acmp_dest_addr, DEFAULT_1722_1_ACMP_SUBTYPE,
                                    message_type, status, AVB_1722_1_ACMP_CD_LENGTH, hdr);
    ...
    memcpy(pkt->dest_mac, cr->stream_dest_mac, MACADDR_NUM_BYTES);
}
...
/*
 * Returns a pointer to the correct inflight command list for a
...
static avb_1722_1_acmp_inflight_command *acmp_get_inflight_list(int entity_type) {
    ...
    switch (entity_type) {
    case CONTROLLER:
        inflight = &acmp_controller_inflight_commands[0];
        break;
    case LISTENER:
        inflight = &acmp_listener_inflight_commands[0];
        break;
    }
...
/*
 * Returns the index into the inflight list for a specific received command's
 * sequence id
 *
 * Returns -1 if the sequence id is not found in the list
 */
static int acmp_get_inflight_from_sequence_id(int entity_type, uint16_t id) {
    int i;
...
/**
 * Sets the timeout field of an inflight command to the current time plus offset
 * for the particular message type
 */
static void acmp_update_inflight_timeout(int entity_type,
                                         avb_1722_1_acmp_inflight_command *inflight,
                                         unsigned int message_type) {
    // Must check the message type is a valid value before doing the timeout
    // array access
    if ((message_type % 2 == 0) && (message_type <= 12)) {
        inflight->timeout = acmp_centisecond_counter[entity_type] +
                            avb_1722_1_acmp_inflight_timeouts[message_type / 2];
    } else {
        inflight->timeout = 0;
    }
}
...
void acmp_add_inflight(int entity_type, unsigned int message_type, uint16_t original_sequence_id) {
    ...
    for (i = 0; i < AVB_1722_1_MAX_INFLIGHT_COMMANDS; i++) {
        ...
        switch (entity_type) {
        case CONTROLLER:
            inflight[i].command = acmp_controller_cmd_resp;
            break;
        case LISTENER:
            inflight[i].command = acmp_listener_rcvd_cmd_resp;
            break;
        }
...
avb_1722_1_acmp_inflight_command *acmp_remove_inflight(int entity_type) {
    ...
    switch (entity_type) {
    case CONTROLLER:
        acmp_command = &acmp_controller_cmd_resp;
        break;
    case LISTENER:
        acmp_command = &acmp_listener_rcvd_cmd_resp;
        break;
    }
    ...
    if (index >= 0) {
        ...
        debug_printf("ACMP %s: Trying to find entry for seq id: %d but it "
                     "doesn't exist\n",
...
int acmp_check_inflight_command_timeouts(int entity_type) {
    ...
    for (i = 0; i < AVB_1722_1_MAX_INFLIGHT_COMMANDS; i++) {
        if (inflight[i].in_use) {
            if (acmp_centisecond_counter[entity_type] >= inflight[i].timeout)
                return i;
        }
    }
...
void acmp_set_talker_response(void) {
    ...
    memcpy(acmp_talker_rcvd_cmd_resp.stream_dest_mac, acmp_talker_streams[talker].destination_mac,
           MACADDR_NUM_BYTES);
...
static void store_rcvd_cmd_resp(avb_1722_1_acmp_cmd_resp *store, avb_1722_1_acmp_packet_t *pkt) {
    ...
    memcpy(store->stream_dest_mac, pkt->dest_mac, MACADDR_NUM_BYTES);
...
void acmp_listener_store_fast_connect_info(int unique_id,
    ...
    if (fl_readDataPage(0, (uint8_t *)avb_1722_1_buf) == 0) {
        avb_1722_1_acmp_fast_connect_persist_state *info =
            (avb_1722_1_acmp_fast_connect_persist_state *)&avb_1722_1_buf[0];
        ...
        fl_writeDataPage(0, (uint8_t *)avb_1722_1_buf);
...
void acmp_listener_erase_fast_connect_info(int unique_id) {
    if (fl_readDataPage(0, (uint8_t *)avb_1722_1_buf) == 0) {
        ...
        if (bitfield_temp != 0xFFFFFFFF) {
            ...
            bitfield_temp &= ~(1 << unique_id);
            info->info_present_bitfield = bitfield_temp;
            memset(&info->talkers[unique_id], 0xFF,
                   sizeof(avb_1722_1_acmp_fast_connect_talker_info));
...
static int acmp_listener_read_fast_connect_info(uint8_t buffer[256]) {
...
void acmp_start_fast_connect(CLIENT_INTERFACE(ethernet_tx_if, i_eth),
    ...
    if (acmp_listener_read_fast_connect_info(flash_buf) == 0) {
        avb_1722_1_acmp_fast_connect_persist_state *info =
            (avb_1722_1_acmp_fast_connect_persist_state *)&flash_buf[0];
        ...
        for (int i = 0; i < AVB_1722_1_MAX_LISTENERS; i++) {
            if ((info->info_present_bitfield >> i) & 1) {
                memcpy(&acmp_listener_rcvd_cmd_resp.controller_guid,
                       &info->talkers[i].controller_guid, sizeof(guid_t));
                memcpy(&acmp_listener_rcvd_cmd_resp.talker_guid, &info->talkers[i].talker_guid,
                       sizeof(guid_t));
...
/**
 * Returns 1 if the listener unique id in a received command is within a valid
 * range.
 *
 * Else returns 0.
 */
...
void acmp_add_listener_stream_info(void) {
    ...
    memcpy(acmp_listener_streams[unique_id].destination_mac,
           acmp_listener_rcvd_cmd_resp.stream_dest_mac, 6);
    ...
    acmp_listener_streams[unique_id].talker_unique_id =
...
avb_1722_1_acmp_status_t acmp_listener_get_state(void) {
    ...
    memcpy(acmp_listener_rcvd_cmd_resp.stream_dest_mac,
           acmp_listener_streams[unique_id].destination_mac, 6);
    ...
    /* If for some reason we couldn't get the state, we could return a
     * STATE_UNVAILABLE status instead */
...
avb_1722_1_acmp_status_t acmp_talker_get_state(void) {
    ...
    memcpy(acmp_talker_rcvd_cmd_resp.stream_dest_mac,
           acmp_talker_streams[unique_id].destination_mac, 6);
...
avb_1722_1_acmp_status_t acmp_talker_get_connection(void) {
    int unique_id = acmp_talker_rcvd_cmd_resp.talker_unique_id;
    uint16_t connection = acmp_talker_rcvd_cmd_resp.connection_count;
    ...
    acmp_talker_rcvd_cmd_resp.listener_guid.l =
        acmp_talker_streams[unique_id].connected_listeners[connection].guid.l;
    acmp_talker_rcvd_cmd_resp.listener_unique_id =
        acmp_talker_streams[unique_id].connected_listeners[connection].unique_id;
    acmp_talker_rcvd_cmd_resp.vlan_id = AVB_DEFAULT_VLAN;
...
void acmp_add_talker_stream_info(void) {
    ...
    // Check listener_guid and listener_unique_id are not in connected_listeners
    // as per 8.2.2.6.2.2. of the spec
    for (i = 0; i < AVB_1722_1_MAX_LISTENERS_PER_TALKER; i++) {
        if (acmp_talker_streams[unique_id].connected_listeners[i].guid.l ==
                acmp_talker_rcvd_cmd_resp.listener_guid.l &&
            acmp_talker_streams[unique_id].connected_listeners[i].unique_id ==
                acmp_talker_rcvd_cmd_resp.listener_unique_id) {
    ...
    for (i = 0; i < AVB_1722_1_MAX_LISTENERS_PER_TALKER; i++) {
        if (acmp_talker_streams[unique_id].connected_listeners[i].guid.l ==
            0) // The first empty connected_listeners field
        {
            acmp_talker_streams[unique_id].connected_listeners[i].guid.l =
                acmp_talker_rcvd_cmd_resp.listener_guid.l;
            acmp_talker_streams[unique_id].connected_listeners[i].unique_id =
                acmp_talker_rcvd_cmd_resp.listener_unique_id;
...
/**
 * Returns 1 if the talker unique id in a received command is within a valid
 * range.
 *
 * Else returns 0.
 */
...
static void process_avb_1722_1_acmp_controller_packet(uint8_t message_type,
                                                      avb_1722_1_acmp_packet_t *pkt) {
    int inflight_index = 0;

    if (acmp_controller_state != ACMP_CONTROLLER_WAITING)
        return;
    if (compare_guid(pkt->controller_guid, &my_guid) == 0)
        return;

    inflight_index = acmp_get_inflight_from_sequence_id(CONTROLLER, ntoh_16(pkt->sequence_id));
    if (inflight_index < 0)
        return; // We don't have an inflight entry for this command

    if (message_type !=
        (acmp_controller_inflight_commands[inflight_index].command.message_type + 1))
        return;
...
static void process_avb_1722_1_acmp_talker_packet(uint8_t message_type,
                                                  avb_1722_1_acmp_packet_t *pkt) {
    if (compare_guid(pkt->talker_guid, &my_guid) == 0)
        return;
    if (acmp_talker_state != ACMP_TALKER_WAITING)
        return;
...
static void process_avb_1722_1_acmp_listener_packet(uint8_t message_type,
                                                    avb_1722_1_acmp_packet_t *pkt) {
    if (compare_guid(pkt->listener_guid, &my_guid) == 0)
        return;
    if (acmp_listener_state != ACMP_LISTENER_WAITING)
        return;
...
void avb_1722_1_talker_set_mac_address(unsigned talker_unique_id, uint8_t macaddr[]) {
    if (talker_unique_id < AVB_1722_1_MAX_TALKERS) {
        memcpy(acmp_talker_streams[talker_unique_id].destination_mac, macaddr, MACADDR_NUM_BYTES);
    }
}

void avb_1722_1_talker_set_stream_id(unsigned talker_unique_id, unsigned streamId[2]) {
    if (talker_unique_id < AVB_1722_1_MAX_TALKERS) {
        acmp_talker_streams[talker_unique_id].stream_id.l =
            (uint64_t)streamId[1] + (((uint64_t)streamId[0]) << 32);
    }
}

...
/*** ADP ***/
void avb_entity_on_new_entity_available_default(client interface avb_interface avb,
...
/* The controller has indicated that a listener is connecting to this talker
 * stream */
void avb_talker_on_listener_connect_default(client interface avb_interface avb,
                                            int source_num,
    ...
    debug_printf("CONNECTING Talker stream #%d (%x%x) -> Listener "
                 "%x:%x:%x:%x:%x:%x:%x:%x\n",
                 source_num, stream_id[0], stream_id[1], listener_guid.c[7], listener_guid.c[6],
                 listener_guid.c[5], listener_guid.c[4], listener_guid.c[3], listener_guid.c[2],
                 listener_guid.c[1], listener_guid.c[0]);

    // If this is the first listener to connect to this talker stream, we do a
    // stream registration to reserve the necessary bandwidth on the network
...
/* The controller has indicated that a listener has returned an error on
 * connection attempt */
void avb_talker_on_listener_connect_failed_default(client interface avb_interface avb,
                                                   const_guid_ref_t my_guid,
                                                   int source_num,
                                                   const_guid_ref_t listener_guid,
...
avb_1722_1_acmp_status_t avb_listener_on_talker_connect_default(client interface avb_interface avb,
    ...
    debug_printf("CONNECTING Listener sink #%d -> Talker stream %x%x, DA: "
                 "%x:%x:%x:%x:%x:%x\n",
                 sink_num, stream_id[0], stream_id[1], dest_addr[0], dest_addr[1], dest_addr[2],
...
/* The controller has indicated to disconnect this listener sink from a talker
 * stream */
void avb_listener_on_talker_disconnect_default(client interface avb_interface avb,
                                               ...
                                               unsigned int stream_id[2],
                                               const_guid_ref_t my_guid) {
    debug_printf("DISCONNECTING Listener sink #%d -> Talker stream %x%x, DA: "
                 "%x:%x:%x:%x:%x:%x\n",
                 sink_num, stream_id[0], stream_id[1], dest_addr[0], dest_addr[1], dest_addr[2],
                 dest_addr[3], dest_addr[4], dest_addr[5]);
...
void avb_talker_on_listener_disconnect_default(client interface avb_interface avb,
    ...
    debug_printf("DISCONNECTING Talker stream #%d (%x%x) -> Listener "
                 "%x:%x:%x:%x:%x:%x:%x:%x\n",
                 source_num, stream_id[0], stream_id[1], listener_guid.c[7], listener_guid.c[6],
                 listener_guid.c[5], listener_guid.c[4], listener_guid.c[3], listener_guid.c[2],
                 listener_guid.c[1], listener_guid.c[0]);

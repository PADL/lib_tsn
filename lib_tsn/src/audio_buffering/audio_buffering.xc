...
static void
init_audio_output_fifos(struct output_finfo &inf, audio_output_fifo_data_t ofifo_data[], size_t n) {
  unsafe {
...
unsafe static audio_frame_t *unsafe
audio_buffers_swap_active_buffer(audio_double_buffer_t &buffer);

[[distributable]] void audio_input_sample_buffer(server push_if i_push, server pull_if i_pull) {
  audio_double_buffer_t input_sample_buf;
  init_audio_input_buffer(input_sample_buf);
  struct input_finfo inf;

  unsafe { inf.p_buffer = &input_sample_buf; }

  while (1) {
      select {
      case i_push.get_handle()->buffer_handle_t res:
          unsafe { res = (void *unsafe) & inf; }
          break;
...
[[distributable]] void audio_output_sample_buffer(server push_if i_push, server pull_if i_pull) {
...
void audio_buffer_manager(streaming chanend c_audio,
  ...
  unsafe {
      ...
      audio_output_fifo_t *unsafe output_sample_buf =
          (audio_output_fifo_t * unsafe)((struct output_finfo *)h_out)->p_buffer;
      media_ctl_register(c_media_ctl, AVB_NUM_MEDIA_INPUTS, output_sample_buf,
                         AVB_NUM_MEDIA_OUTPUTS, 0);
...
#pragma loop unroll
                    for (size_t i = 0; i < AVB_NUM_MEDIA_OUTPUTS; i += 2) {
                        sample_out_buf[i] =
                            audio_output_fifo_pull_sample(h_out, i, timestamp, valid);
                    }
#pragma loop unroll
...
#pragma loop unroll
                for (size_t i = 0; i < AVB_NUM_SINKS;
                     i++) { // FIXME: This should be number of TDM lines
                    size_t index = channel + (i * 8);
                    sample_out_buf[i] =
                        audio_output_fifo_pull_sample(h_out, index, timestamp, valid);
                }
                c_audio <: (int32_t *unsafe)&sample_out_buf;
                channel++;

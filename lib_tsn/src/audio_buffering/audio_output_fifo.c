...
void audio_output_fifo_init(buffer_handle_t s0, unsigned index) {
...
void disable_audio_output_fifo(buffer_handle_t s0, unsigned index) {
    ofifo_t *s = (ofifo_t *)((struct output_finfo *)s0)->p_buffer[index];
...
void enable_audio_output_fifo(buffer_handle_t s0, unsigned index, int media_clock) {
    ofifo_t *s = (ofifo_t *)((struct output_finfo *)s0)->p_buffer[index];
...
// 1722 thread
void audio_output_fifo_maintain(buffer_handle_t s0,
                                unsigned index,
                                chanend buf_ctl,
    ...
    if (s->pending_init_notification && !(*notified_buf_ctl)) {
        notify_buf_ctl_of_new_stream(buf_ctl,
                                     (int)s); // TODO: This can pass the index
        *notified_buf_ctl = 1;
        s->pending_init_notification = 0;
    }
    ...
    case LOCKING:
    case LOCKED:
        time_since_last_notification = (signed)s->sample_count - (signed)s->last_notification_time;
        if (s->ptp_ts != 0 && s->local_ts != 0 && !(*notified_buf_ctl) &&
            (s->last_notification_time == 0 ||
...
// 1722 thread
void audio_output_fifo_strided_push(
    buffer_handle_t s0,
    size_t index,
    uint8_t *sample_ptr,
    ...
    for (size_t i = 0; i < num_samples; i += stride) {
        ...
        if (new_wrptr != s->dptr) {
            *wrptr = sample & valid_mask;
            ...
            // debug_printf("audio_output_fifo_strided_push: overflow index
            // %u\n", index);
        }
    }
...
// 1722 thread
void audio_output_fifo_handle_buf_ctl(
    chanend buf_ctl, buffer_handle_t s0, unsigned index, int *buf_ctl_notified, timer tmr) {
    int cmd;
    ofifo_t *s = (ofifo_t *)((struct output_finfo *)s0)->p_buffer[index];
    cmd = get_buf_ctl_cmd(buf_ctl);
    switch (cmd) {
    case BUF_CTL_REQUEST_INFO: {
        send_buf_ctl_info(buf_ctl, s->state == LOCKED, s->ptp_ts, s->local_ts,
                          s->dptr - START_OF_FIFO(s), s->wrptr - START_OF_FIFO(s), tmr);
        s->ptp_ts = 0;
        s->local_ts = 0;
        s->marker = (unsigned int *)0;
        break;
    ...
    case BUF_CTL_REQUEST_NEW_STREAM_INFO: {
        send_buf_ctl_new_stream_info(buf_ctl, s->media_clock);
        buf_ctl_ack(buf_ctl);
        *buf_ctl_notified = 0;
        break;
    }

...
static void register_talkers(chanend (&?c_talker_ctl)[], uint8_t mac_addr[MACADDR_NUM_BYTES])
{
    unsafe {
      ...
      for (size_t j = 0; j < num_streams; j++) {
        ...
        source->reservation.stream_id[0] =
            (mac_addr[0] << 24) | (mac_addr[1] << 16) | (mac_addr[2] << 8) | (mac_addr[3] << 0);
        source->reservation.stream_id[1] =
            (mac_addr[4] << 24) | (mac_addr[5] << 16) | ((source->stream.local_id & 0xffff) << 0);
        source->presentation = AVB_DEFAULT_PRESENTATION_TIME_DELAY_NS;
        source->reservation.vlan_id = 0;
        source->reservation.tspec =
            (AVB_SRP_TSPEC_PRIORITY_DEFAULT << 5 | AVB_SRP_TSPEC_RANK_DEFAULT << 4 |
...
static void register_media(chanend media_ctl[]) {
    unsafe {
      ...
      for (size_t j = 0; j < num_in; j++) {
        media_ctl[i]<:input_id; inputs[input_id].tile_id = tile_id;
                      inputs[input_id].clk_ctl = clk_ctl; inputs[input_id].local_id = j;
                      inputs[input_id].mapped_to = UNMAPPED; media_ctl[i]:> inputs[input_id]
            .fifo;
        input_id++;
      }
      media_ctl[i] :> num_out;
      for (size_t j = 0; j < num_out; j++) {
        media_ctl[i]<:output_id; outputs[output_id].tile_id = tile_id;
                      outputs[output_id].clk_ctl = clk_ctl; outputs[output_id].local_id = j;
                      outputs[output_id].mapped_to = UNMAPPED; media_ctl[i]:> outputs[output_id]
            .fifo;
        output_id++;
      }
        }
...
static void init_media_clock_server(client interface media_clock_if media_clock_ctl) {
...
static void update_sink_state(unsigned sink_num,
    ...
    if (prev == AVB_SINK_STATE_DISABLED && state == AVB_SINK_STATE_POTENTIAL) {
      ...
      master { *c<:AVB1722_GET_ROUTER_LINK; *c:> router_link; }
      ...
      if (isnull(i_srp)) {
        debug_printf("MSRP: Register attach request %x:%x\n", sink->reservation.stream_id[0],
                      sink->reservation.stream_id[1]);
        sink->reservation.vlan_id =
            avb_srp_join_listener_attrs(sink->reservation.stream_id, sink->reservation.vlan_id);
      } else {
        sink->reservation.vlan_id =
            i_srp.register_attach_request(sink->reservation.stream_id, sink->reservation.vlan_id);
      }

    } else if (prev != AVB_SINK_STATE_DISABLED && state != AVB_SINK_STATE_DISABLED) {
...
static void
configure_talker_stream(chanend c, avb_source_info_t *alias source, unsigned source_num) {
...
static unsigned avb_srp_calculate_max_framesize(avb_source_info_t *source_info) {
    ...
    const unsigned samples_per_packet =
        (AVB_MAX_AUDIO_SAMPLE_RATE + (AVB1722_PACKET_RATE - 1)) / AVB1722_PACKET_RATE;
    return AVB1722_PLUS_SIP_HEADER_SIZE +
           (source_info->stream.num_channels * samples_per_packet * 4);
#else
...
static void update_source_state(unsigned source_num,
    ...
    if (prev == AVB_SOURCE_STATE_DISABLED && state == AVB_SOURCE_STATE_POTENTIAL) {
      // enable the source
      int valid = 1;
      ...
      if (valid) {
        ...
        if (isnull(i_srp)) {
            debug_printf("MSRP: Register stream request %x:%x\n", source->reservation.stream_id[0],
                          source->reservation.stream_id[1]);
            source->reservation.vlan_id =
                avb_srp_create_and_join_talker_advertise_attrs(&source->reservation);
        } else {
    ...
    } else if (prev == AVB_SOURCE_STATE_ENABLED && state == AVB_SOURCE_STATE_POTENTIAL) {
      // stop transmission
    ...
    } else if (prev == AVB_SOURCE_STATE_POTENTIAL && state == AVB_SOURCE_STATE_ENABLED) {
      // start transmitting
      configure_talker_stream(*c, source, source_num);
    ...
    } else if (prev != AVB_SOURCE_STATE_DISABLED && state == AVB_SOURCE_STATE_DISABLED) {
      // disabled the source
      for (size_t i = 0; i < source->stream.num_channels; i++) {
...
static void get_debug_counters(struct avb_debug_counters &counters) {
    ...
    unsafe {
      ...
      master { *c<:AVB1722_GET_COUNTERS; *c:> tc; }
    }
    ...
    unsafe {
      ...
      master { *c<:AVB1722_GET_COUNTERS; *c:> lc; }
    }
...
int avb_get_source_state(client interface avb_interface avb,
                         unsigned source_num,
                         enum avb_source_state_t &state) {
    return avb.get_source_state(source_num, state);
}

int avb_set_source_state(client interface avb_interface avb,
                         unsigned source_num,
                         enum avb_source_state_t state) {
...
void avb_manager(server interface avb_interface avb[num_avb_clients], unsigned num_avb_clients,
    ...
    unsafe { avb_init(c_media_ctl, c_listener_ctl, c_talker_ctl, i_media_clock_ctl, i_eth_cfg); }
    ...
    case avb[int i]._set_sink_info(unsigned sink_num, avb_sink_info_t info):
      ...
      sinks[sink_num] = info;
      unsafe {
          update_sink_state(sink_num, prev_state, info.stream.state, i_eth_cfg, i_media_clock_ctl,
                            i_srp);
      }
      break;
    case avb[int i]._get_media_clock_info(unsigned clock_num)->media_clock_info_t info:
      info = i_media_clock_ctl.get_clock_info(clock_num);
      break;
    case avb[int i]._set_media_clock_info(unsigned clock_num, media_clock_info_t info):
      ...
      break;
    case avb[int i]._get_debug_counters(void)->struct avb_debug_counters counters:
...
void avb_process_1722_control_packet(unsigned int buf0[],
    ...
    if (has_qtag) {
      ...
      etype = (int)(tagged_ethernet_hdr->ethertype.data[0] << 8) +
              (int)(tagged_ethernet_hdr->ethertype.data[1]);
    } else {

...
void ptp_recv(
    client interface ethernet_tx_if, uint8_t buf[], unsigned ts, unsigned src_port, unsigned len);
...
void ptp_process_client_request(chanend c, timer ptp_timer) {
  ...
  case PTP_GET_AS_PATH: {
    master {
        ...
        ptp_get_path_sequence(&count, pathSequence);
        c <: count;
...
void ptp_server(client interface ethernet_rx_if i_eth_rx,
  ...
  while (1) {
    select { do_ptp_server(i_eth_rx, i_eth_tx, ptp_clients, num_clients, ptp_timer, ptp_timeout); }
  }
}

...
void ptp_get_reference_ptp_ts_mod_64(unsigned &hi, unsigned &lo) {
    uint64_t t;
    t = ptp_reference_ptp_ts.seconds[0] + ((uint64_t)ptp_reference_ptp_ts.seconds[1] << 32);
...
static void
ptp_timestamp_offset64(ptp_timestamp &alias dst, ptp_timestamp &alias ts, long long offset) {
...
void ptp_timestamp_offset(ptp_timestamp &ts, int offset) { ptp_timestamp_offset64(ts, ts, offset); }
...
void local_timestamp_to_ptp(ptp_timestamp &ptp_ts, unsigned local_ts, ptp_time_info &info) {
    _local_timestamp_to_ptp(ptp_ts, local_ts, info.local_ts, info.ptp_ts, info.ptp_adjust);
}

#define local_to_ptp_ts(ptp_ts, local_ts)                                                          \
    _local_timestamp_to_ptp(ptp_ts, local_ts, ptp_reference_local_ts, ptp_reference_ptp_ts,        \
                            g_ptp_adjust)
...
static int update_adjust(ptp_timestamp &master_ts, unsigned local_ts) {
    ...
    if (prev_adjust_valid) {
        ...
        if (g_ptp_adjust_valid) {
            ...
            adjust =
                (((long long)g_ptp_adjust) * (PTP_ADJUST_WEIGHT - 1) + adjust) / PTP_ADJUST_WEIGHT;

            g_ptp_adjust = (int)adjust;

            inv_adjust = (((long long)g_inv_ptp_adjust) * (PTP_ADJUST_WEIGHT - 1) + inv_adjust) /
                         PTP_ADJUST_WEIGHT;
...
static void periodic_update_reference_timestamps(unsigned int local_ts) {
    ...
    if (local_diff > UPDATE_REFERENCE_TIMESTAMP_PERIOD) {
        ...
        ptp_timestamp_offset64(ptp_reference_ptp_ts, ptp_reference_ptp_ts, ptp_diff);
    }
}
...
static void update_path_delay(ptp_timestamp &master_ingress_ts,
    ...
    /* The sequence of events is:
       ...
       ((local_ingress_ts - local_egress_ts) - (master_egress_ts -
       master_ingress_ts) ) / 2
    ...
    */
    ...
    if (port_info.delay_info.valid) {
        ...
        port_info.delay_info.pdelay =
            ((port_info.delay_info.pdelay * (PTP_PATH_DELAY_WEIGHT - 1)) + (int)delay) /
...
static int compare_clock_identity(n64_t *c1, n64_t *c2) {
    for (int i = 0; i < 8; i++) {
...
static void bmca_update_roles(char *msg, unsigned t, int port_num) {
    ...
    AnnounceMessage *pAnnounceMesg =
        (AnnounceMessage *)((char *)pComMesgHdr + sizeof(ComMessageHdr));
    int clock_identity_comp;
    int new_best = 0;

    clock_identity_comp = compare_clock_identity_to_me(&pAnnounceMesg->grandmasterIdentity);
    ...
    if (clock_identity_comp == 0) {
        ...
        if (pAnnounceMesg->grandmasterPriority1 > best_announce_msg.grandmasterPriority1) {
            ...
            new_best = 1;
                   ...
                   ntoh16(best_announce_msg.clockOffsetScaledLogVariance)) {
            new_best = -1;
        } else if (ntoh16(pAnnounceMesg->clockOffsetScaledLogVariance) <
                   ntoh16(best_announce_msg.clockOffsetScaledLogVariance)) {
            new_best = 1;
            ...
            clock_identity_comp = compare_clock_identity(&pAnnounceMesg->grandmasterIdentity,
...
static void network_to_ptp_timestamp(ptp_timestamp &ts, n80_t &msg) {
    char *sec0_p = (char *)&ts.seconds[0];
    char *sec1_p = (char *)&ts.seconds[1];
    char *nsec_p = (char *)&ts.nanoseconds;
...
static void
ptp_tx(client interface ethernet_tx_if i_eth, unsigned int *buf, int len, int port_num) {
    len = len < 64 ? 64 : len;
...
static uint8_t src_mac_addr[6];
static uint8_t dest_mac_addr[6] = PTP_DEFAULT_DEST_ADDR;
...
static void create_my_announce_msg(AnnounceMessage *pAnnounceMesg) {
    ...
    pAnnounceMesg->clockOffsetScaledLogVariance = hton16(PTP_OFFSET_SCALED_LOG_VARIANCE);
...
#define ANNOUNCE_PACKET_SIZE                                                                       \
    (sizeof(ethernet_hdr_t) + sizeof(ComMessageHdr) + sizeof(AnnounceMessage))
    ...
    pComMesgHdr->transportSpecific_messageType = PTP_TRANSPORT_SPECIFIC_HDR | PTP_ANNOUNCE_MESG;

    pComMesgHdr->versionPTP = PTP_VERSION_NUMBER;

    pComMesgHdr->flagField[1] = ((PTP_LEAP61 & 0x1)) | ((PTP_LEAP59 & 0x1) << 1) |
...
#define FOLLOWUP_PACKET_SIZE                                                                       \
    (sizeof(ethernet_hdr_t) + sizeof(ComMessageHdr) + sizeof(FollowUpMessage))
    ...
    pComMesgHdr->transportSpecific_messageType = PTP_TRANSPORT_SPECIFIC_HDR | PTP_SYNC_MESG;

    pComMesgHdr->versionPTP = PTP_VERSION_NUMBER;

    pComMesgHdr->messageLength = hton16(sizeof(ComMessageHdr) + sizeof(SyncMessage));
    ...
    ptp_tx_timed(i_eth, buf0, SYNC_PACKET_SIZE, local_egress_ts, port_num);
    ...
    pComMesgHdr->transportSpecific_messageType = PTP_TRANSPORT_SPECIFIC_HDR | PTP_FOLLOW_UP_MESG;

    pComMesgHdr->controlField = PTP_CTL_FIELD_FOLLOW_UP;

    pComMesgHdr->messageLength = hton16(sizeof(ComMessageHdr) + sizeof(FollowUpMessage));
...
#define PDELAY_REQ_PACKET_SIZE                                                                     \
    (sizeof(ethernet_hdr_t) + sizeof(ComMessageHdr) + sizeof(PdelayReqMessage))
    ...
    pComMesgHdr->transportSpecific_messageType = PTP_TRANSPORT_SPECIFIC_HDR | PTP_PDELAY_REQ_MESG;
    ...
    ptp_tx_timed(i_eth, buf0, PDELAY_REQ_PACKET_SIZE, pdelay_request_sent_ts[port_num], port_num);
...
static void send_ptp_pdelay_resp_msg(client interface ethernet_tx_if i_eth,
                                     ...
                                     int port_num) {
#define PDELAY_RESP_PACKET_SIZE                                                                    \
    (sizeof(ethernet_hdr_t) + sizeof(ComMessageHdr) + sizeof(PdelayRespMessage))
    ...
    pTxMesgHdr->messageLength = hton16(sizeof(ComMessageHdr) + sizeof(PdelayRespMessage));
    ...
    pTxMesgHdr->transportSpecific_messageType = PTP_TRANSPORT_SPECIFIC_HDR | PTP_PDELAY_RESP_MESG;

    local_to_epoch_ts(req_ingress_ts, &epoch_req_ingress_ts);

    timestamp_to_network(pTxRespHdr->requestReceiptTimestamp, epoch_req_ingress_ts);
    ...
    timestamp_to_network(pTxFollowUpHdr->responseOriginTimestamp, epoch_resp_ts);
...
static int
qualify_announce(ComMessageHdr &alias header, AnnounceMessage &alias announce_msg, int this_port) {
    for (int i = 0; i < 8; i++) {
...
void ptp_recv(client interface ethernet_tx_if i_eth,
    ...
    case PTP_SYNC_MESG:
        ...
        if (asCapable && !received_sync && ptp_port_info[src_port].role_state == PTP_SLAVE) {
            received_sync = 1;
    ...
    case PTP_FOLLOW_UP_MESG:
        if ((received_sync == 1) &&
            ...
            if (received_sync_id == ntoh16(msg->sequenceId)) {
                ...
                network_to_ptp_timestamp(master_egress_ts, follow_up_msg->preciseOriginTimestamp);

                ptp_timestamp_offset64(master_egress_ts, master_egress_ts, correction >> 16);
    ...
    case PTP_PDELAY_RESP_MESG:
        ...
        if (!pdelay_request_sent[src_port] && received_pdelay[src_port] &&
            ...
            if (!ptp_port_info[src_port].delay_info.multiple_resp_count ||
                (pdelay_req_seq_id[src_port] ==
                 ptp_port_info[src_port].delay_info.last_multiple_resp_seq_id + 1)) {
        ...
        if (received_pdelay[src_port] && pdelay_req_seq_id[src_port] == ntoh16(msg->sequenceId)) {
    ...
    case PTP_PDELAY_RESP_FOLLOW_UP_MESG:
        if (received_pdelay[src_port]) {
            if (received_pdelay_id[src_port] == ntoh16(msg->sequenceId) &&
                ...
                PdelayRespFollowUpMessage *follow_up_msg = (PdelayRespFollowUpMessage *)(msg + 1);

                network_to_ptp_timestamp(pdelay_resp_egress_ts,
                                         follow_up_msg->responseOriginTimestamp);

                update_path_delay(pdelay_request_receipt_ts[src_port], pdelay_resp_egress_ts,
                                  pdelay_request_sent_ts[src_port],
                                  pdelay_resp_ingress_ts[src_port], ptp_port_info[src_port]);
...
void ptp_init(client interface ethernet_cfg_if i_eth_cfg,
    ...
    if (server_tile_id != get_tile_id_from_chanend(c)) {
        tile_timer_offset = other_tile_now - this_tile_now -
...
void ptp_periodic(client interface ethernet_tx_if i_eth, unsigned t) {
    for (int i = 0; i < PTP_NUM_PORTS; i++) {
        ...
        int recv_sync_timeout_interval =
            last_receive_sync_upstream_interval[i] * PTP_SYNC_RECEIPT_TIMEOUT_MULTIPLE;
        ...
        if (asCapable && role == PTP_MASTER && timeafter(t, last_sync_time[i] + SYNC_PERIOD)) {

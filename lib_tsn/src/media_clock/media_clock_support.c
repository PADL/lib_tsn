 ...
 */
static unsigned int local_wordlen_to_external_wordlen(uint64_t w) {
    return (w >> (WORDLEN_FRACTIONAL_BITS - WC_FRACTIONAL_BITS));
}

static uint64_t calculate_wordlen(unsigned int sample_rate) {
    const uint64_t nanoseconds = (100000000LL << WORDLEN_FRACTIONAL_BITS);
...
unsigned int update_media_clock(
    chanend ptp_svr, int clock_index, const media_clock_t *mclock, unsigned int t2, int period0) {
    clock_info_t *clock_info = &clock_states[clock_index];
    ...
    case DEVICE_MEDIA_CLOCK_INPUT_STREAM_DERIVED: {
        ...
        // If the stream info isn't valid at all, then return the default clock
        // rate
        if (!clock_info->stream_info2.valid)
            return local_wordlen_to_external_wordlen(clock_info->wordlen);

        // If there are not two stream infos to compare, then return default
        // clock rate
        ...
        if (!clock_info->stream_info2.locked) {
            ...
            diff_local = clock_info->stream_info2.local_ts - clock_info->stream_info1.local_ts;
            ...
            clock_info->wordlen = clock_info->wordlen - ((perror / diff_local) * 80) / 11 -

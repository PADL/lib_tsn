...
void update_stream_derived_clocks(int source_num,
    ...
    if (media_clocks[i].info.active &&
        ...
        update_media_clock_stream_info(i, local_ts, ptp_outgoing_actual, presentation_timestamp,
                                       locked, fill);
    }
    }
}
...
static void init_buffers(void) {
    // This used to set the (now removed) adjust field in the buf_into_t array
    // to zero
}
                          ...
                          timer tmr)
{
  ...
  master {
        buf_ctl<:0; buf_ctl:> othercore_now;
    tmr :> thiscore_now;
    buf_ctl :> fifo_locked;
  ...
  if (!fifo_locked && (b.stability_count > STABLE_THRESHOLD)) {
        ...
        if (fill - sample_diff > max_adjust || fill - sample_diff < -max_adjust) {
...
static void update_media_clock_divide(media_clock_t &clk) {
  uint64_t divWordLength = (uint64_t)clk.wordLength * INTERNAL_CLOCK_DIVIDE / 2;
  clk.baseLength = divWordLength >> (WC_FRACTIONAL_BITS);
  clk.baseLengthRemainder = divWordLength & ((1 << WC_FRACTIONAL_BITS) - 1);
}

static void init_media_clock(media_clock_t &clk, timer tmr, out buffered port:32 p) {
  int ptime, time;
  clk.info.active = 0;
  clk.count = 0;
  ...
  clk.next_event = time + INITIAL_MEDIA_CLOCK_OUTPUT_DELAY + EVENT_AFTER_PORT_OUTPUT_DELAY;
}

static void do_media_clock_output(media_clock_t &clk, out buffered port:32 p) {
  ...
  for (int i = 0; i < AVB_NUM_MEDIA_CLOCKS; i++) {
      if (media_clocks[i].info.active) {
          media_clocks[i].wordLength =
              update_media_clock(ptp_svr, i, media_clocks[i], clk_time, CLOCK_RECOVERY_PERIOD);
...
#pragma ordered
      ...
      case (int i = 0; i < num_ptp; i++) ptp_process_client_request(c_ptp[i], tmr):
          break;
...
#if (AVB_NUM_MEDIA_OUTPUTS != 0)
      case (int i = 0; i < num_buf_ctl; i++)(fifo_init_count == 0)
        => inuchar_byref(buf_ctl[i], buf_ctl_cmd): {
          int fifo, buf_index;
          unsigned x;
#if defined(__XS2A__)
          ...
          case BUF_CTL_GOT_INFO:
              manage_buffer(buf_info[buf_index], ptp_svr, buf_ctl[i], buf_index, tmr);
              break;
      ...
      case media_clock_ctl.register_clock(unsigned i, unsigned clock_num):
          ...
          break;
      case media_clock_ctl.get_clock_info(unsigned clock_num)->media_clock_info_t info:
          info = media_clocks[clock_num].info;
          break;
      case media_clock_ctl.set_clock_info(unsigned clock_num, media_clock_info_t info):
          ...
          if (!prev_active && info.active) {
              init_media_clock_recovery(ptp_svr, clock_num, clk_time - CLOCK_RECOVERY_PERIOD,

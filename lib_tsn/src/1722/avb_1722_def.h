...
typedef struct
{
  ...
  uint8_t DBS;                    // Data block size. In 61883 a data block is all information arriving
...
static inline uint8_t bit_depth_from_format(uint8_t format) {
    switch (format) {
    case AAF_FORMAT_INT_32BIT: return 32;
    case AAF_FORMAT_INT_24BIT: return 24;
    case AAF_FORMAT_INT_16BIT: return 16;
    default: return 0;
    }
}
...
static inline uint8_t format_from_bit_depth(uint8_t bit_depth) {
    switch (bit_depth) {
    case 32: return AAF_FORMAT_INT_32BIT;
    case 24: return AAF_FORMAT_INT_24BIT;
    case 16: return AAF_FORMAT_INT_16BIT;
    default: return AAF_FORMAT_USER;
    }
}
...
static inline uint8_t nsr_from_sampling_rate(int rate) {
    switch (rate) {
#define NSR_FROM_SAMPLING_RATE_CASE(_rate) case _rate: return AAF_NSR_##_rate
    ...
    default: return AAF_NSR_USER_SPECIFIED;
    }
}

static inline int sampling_rate_from_nsr(uint8_t nsr) {
    switch (nsr) {
#define SAMPLING_RATE_FROM_NSR_CASE(_rate) case AAF_NSR_##_rate: return _rate

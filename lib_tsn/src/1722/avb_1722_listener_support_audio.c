...
static int avb_1722_listener_process_packet_61883(chanend buf_ctl,
    ...
    if (stream_info->chan_lock < 16) {
        ...
        if (stream_info->chan_lock == 16) {
            ...
            case 1:
                stream_info->rate = 8000;
                break;
            case 2:
                stream_info->rate = 16000;
                break;
            case 4:
                stream_info->rate = 32000;
                break;
            case 5:
                stream_info->rate = 44100;
                break;
            case 6:
                stream_info->rate = 48000;
                break;
            case 11:
                stream_info->rate = 88200;
                break;
            case 12:
                stream_info->rate = 96000;
                break;
            case 24:
                stream_info->rate = 192000;
                break;
            default:
                stream_info->rate = 0;
                break;
    ...
    if (AVBTP_TV(pAVBHdr)) {
        // See 61883-6 section 6.2 which explains that the receiver can
        // calculate which data block (sample) the timestamp refers to using the
        // formula:
        ...
        for (size_t i = 0; i < num_channels; i++) {
            if (map[i] >= 0)
                audio_output_fifo_set_ptp_timestamp(h, map[i], AVBTP_TIMESTAMP(pAVBHdr),
                                                    sample_num);
        }
    }
...
static int avb_1722_listener_process_packet_aaf(chanend buf_ctl,
                                                AVB_DataHeader_t *pAVBHdr,
                                                size_t pktDataLength,
                                                avb_1722_stream_info_t *stream_info,
                                                ptp_time_info_mod64 *timeInfo,
...
#if AVB_1722_RECORD_ERRORS
        debug_printf("avb_1722_listener_process_packet_aaf: unsupported NSR "
                     "value %02x\n",
                     nsr);
#endif
        return 0;
    ...
    if (AVBTP_TV(pAVBHdr)) {
        if (AVBTP_PROTOCOL_SPECIFIC(pAVBHdr) & AAF_PROTOCOL_SPECIFIC_SPARSE) {
            // TODO: handle sparse timestamps, MILAN doesn't require it so we
            // ignore for now
#if AVB_1722_RECORD_ERRORS
            debug_printf("avb_1722_listener_process_packet_aaf: sparse "
    ...
    size_t num_channels = stream_info->num_channels < stream_info->num_channels_in_payload
                              ...
                              : stream_info->num_channels_in_payload;

    for (size_t i = 0; i < num_channels; i++) {
        if (map[i] >= 0)
            audio_output_fifo_strided_push(h, map[i], sample_ptr, sample_length, cpf,
                                           num_samples_in_payload, AVTP_AUDIO_SUBTYPE, valid_mask);
...
int avb_1722_listener_process_packet(chanend buf_ctl,
...
#if AVB_1722_RECORD_ERRORS
        debug_printf("avb_1722_listener_process_packet: got %u bytes, expected "
                     "at least %u\n",
...
#if AVB_1722_RECORD_ERRORS
        debug_printf("avb_1722_listener_process_packet: received control "
                     "instead of data packet\n");
#endif
        return 0;
    }
...
#if AVB_1722_RECORD_ERRORS
        debug_printf("avb_1722_listener_process_packet: packet truncated, "
                     "expected at least %u bytes, got %u\n",
    ...
    if (stream_info->last_sequence && seq_num - stream_info->last_sequence != 1) {
        debug_printf("avb_1722_listener_process_packet: dropping packet %d "
                     "last %d ts %x\n",
    ...
    case IEC_61883_IIDC_SUBTYPE:
        return avb_1722_listener_process_packet_61883(buf_ctl, pAVBHdr, pktDataLength, stream_info,
                                                      timeInfo, index, notified_buf_ctl, h);
#endif
#ifdef AVB_1722_FORMAT_AAF
    case AVTP_AUDIO_SUBTYPE:
        return avb_1722_listener_process_packet_aaf(buf_ctl, pAVBHdr, pktDataLength, stream_info,
                                                    timeInfo, index, notified_buf_ctl, h);
#endif
    default:
#if AVB_1722_RECORD_ERRORS

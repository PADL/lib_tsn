...
short ntoh_16(uint8_t x[2]);
int ntoh_32(uint8_t x[4]);
...
#define NTOH_U32(x)                                                                                \
    (((unsigned)x[0] << 24) | ((unsigned)x[1] << 16) | ((unsigned)x[2] << 8) | ((unsigned)x[3]))
#define NTOH_U32i(x, i)                                                                            \
    (((unsigned)x[i] << 24) | ((unsigned)x[i + 1] << 16) | ((unsigned)x[i + 2] << 8) |             \
     ((unsigned)x[i + 3]))
...
// AVB frame header
typedef struct {
    uint8_t DA[MAC_ADRS_BYTE_COUNT]; // dest MAC addr
    uint8_t SA[MAC_ADRS_BYTE_COUNT]; // src MAC addr
    uint8_t TPID[2];                 // 802.1P/Q = 8100
    uint8_t flagVID[2];              // contains PCP (priority code point), bits 0,1,2.
...
#define SET_AVBTP_PCP(x, a)                                                                        \
    do {                                                                                           \
        x->flagVID[0] |= (a << 5);                                                                 \
    } while (0)
#define SET_AVBTP_CFI(x, a)                                                                        \
    do {                                                                                           \
        x->flagVID[0] |= (a & 0x1) << 4;                                                           \
    } while (0)
#define SET_AVBTP_VID(x, a)                                                                        \
    do {                                                                                           \
        x->flagVID[0] |= ((a >> 8) & 0xF);                                                         \
        x->flagVID[1] = (a & 0xFF);                                                                \
    } while (0)
#define SET_AVBTP_TPID(x, a)                                                                       \
    do {                                                                                           \
        x->TPID[0] = (a >> 8);                                                                     \
        x->TPID[1] = a & 0xFF;                                                                     \
    } while (0)
#define SET_AVBTP_ETYPE(x, a)                                                                      \
    do {                                                                                           \
        x->etype[0] = (a >> 8);                                                                    \
        x->etype[1] = a & 0xFF;                                                                    \
    } while (0)

#define CLEAR_AVBTP_VID(x)                                                                         \
    do {                                                                                           \
        x->flagVID[0] &= 0xF0;                                                                     \
        x->flagVID[1] = 0;                                                                         \
    } while (0)
...
typedef struct {
    uint8_t subtype;               // bit 0   : cd (control/data indicator). data (0),
                                   // control(1) bit 1-7 : subtype
    uint8_t version_flags;         // bit 0   : sv. stream id field valid.
                                   // bit 1-3 : version.
    ...
    uint8_t reserved_tu;           // bit 0-6 : reserved
                                   // bit 7   : tu. timestamp uncertain.
    uint8_t stream_id[8];          // 802.1Qat Stream ID
    uint8_t avb_timestamp[4];      //
    uint8_t gateway_info[4];       //
    uint8_t packet_data_length[2]; // length of data following the protocol
                                   // specific packet header. Max value 1476
    uint8_t protocol_specific[2];
    // For 61883
    // uint8_t tag_channel;     // bit 0-1 : tag
    //                                // bit 2-7 : channel
    // uint8_t tcode_sy;        // bot 0-3 : tcode
...
#define AVBTP_TIMESTAMP(x)                                                                         \
    ((x->avb_timestamp[0] << 24) | (x->avb_timestamp[1] << 16) | (x->avb_timestamp[2] << 8) |      \
     (x->avb_timestamp[3]))
#define AVBTP_STREAM_ID1(x)                                                                        \
    ((x->stream_id[0] << 24) | (x->stream_id[1] << 16) | (x->stream_id[2] << 8) | (x->stream_id[3]))
#define AVBTP_STREAM_ID0(x)                                                                        \
    ((x->stream_id[4] << 24) | (x->stream_id[5] << 16) | (x->stream_id[6] << 8) | (x->stream_id[7]))
#define AVBTP_PROTOCOL_SPECIFIC(x) ((x->protocol_specific[0] << 8) | (x->protocol_specific[1]))
...
#define SET_AVBTP_PACKET_DATA_LENGTH(x, a)                                                         \
    do {                                                                                           \
        (x)->packet_data_length[0] = (a) >> 8;                                                     \
        (x)->packet_data_length[1] = (a)&0xFF;                                                     \
    } while (0)
#define SET_AVBTP_PROTOCOL_SPECIFIC(x, a)                                                          \
    do {                                                                                           \
        (x)->protocol_specific[0] = (a) >> 8;                                                      \
        (x)->protocol_specific[1] = (a)&0xFF;                                                      \
    } while (0)

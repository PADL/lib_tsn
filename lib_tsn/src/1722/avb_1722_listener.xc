...
#define MAX_PKT_BUF_SIZE_LISTENER                                                                  \
    (AVB_ETHERNET_HDR_SIZE + AVB_TP_HDR_SIZE + AVB_CIP_HDR_SIZE +                                  \
     AVB1722_LISTENER_MAX_NUM_SAMPLES_PER_CHANNEL * AVB_MAX_CHANNELS_PER_LISTENER_STREAM * 4 + 2)

static transaction configure_stream(chanend c, avb_1722_stream_info_t &s, buffer_handle_t h) {
    int media_clock;
    ...
    for (size_t i = 0; i < s.num_channels; i++) {
        c :> s.map[i];
        if (s.map[i] >= 0) {
            unsafe { enable_audio_output_fifo(h, s.map[i], media_clock); }
        }
    }
...
static transaction adjust_stream(chanend c, avb_1722_stream_info_t &s, buffer_handle_t h) {
    int cmd;
    c :> cmd;
    switch (cmd) {
    ...
    case AVB1722_ADJUST_LISTENER_VOLUME: {
        ...
        for (size_t i = 0; i < count; i++) {
            c :> volume;
            if (i < s.num_channels)
                audio_output_fifo_set_volume(h, s.map[i], volume);
        }
#endif
        break;
    }
...
static void disable_stream(avb_1722_stream_info_t &s, buffer_handle_t h) {
    for (size_t i = 0; i < s.num_channels; i++) {
        if (s.map[i] >= 0) {
            unsafe { disable_audio_output_fifo(h, s.map[i]); }
        }
    }
...
void avb_1722_listener_init(chanend c_listener_ctl,
    ...
    for (size_t i = 0; i < MAX_AVB_STREAMS_PER_LISTENER; i++) {
                                     ...
                                     buffer_handle_t h)
{
    ...
    if (stream_id < MAX_AVB_STREAMS_PER_LISTENER && st.listener_streams[stream_id].active) {
        // process the current packet
        avb_1722_listener_process_packet(c_buf_ctl, &(rxbuf, uint8_t[])[2], packet_info.len,
                                         st.listener_streams[stream_id], timeInfo, stream_id,
                                         st.notified_buf_ctl, h);
...
void avb_1722_listener_handle_cmd(chanend c_listener_ctl,
    ...
    slave {
        ...
        case AVB1722_CONFIGURE_LISTENER_STREAM: {
            ...
            configure_stream(c_listener_ctl, st.listener_streams[stream_num], h);
            break;
        }
        case AVB1722_ADJUST_LISTENER_STREAM: {
            int stream_num;
            c_listener_ctl :> stream_num;
            adjust_stream(c_listener_ctl, st.listener_streams[stream_num], h);
            break;
        }
                       ...
                       client push_if audio_output_buf)
{
    ...
    while (1) {
        ...
        case ethernet_receive_hp_packet(c_eth_rx_hp, &(rxbuf, uint8_t[])[2], packet_info):
            avb_1722_listener_handle_packet(rxbuf, packet_info, c_buf_ctl, st, timeInfo, h);
            break;

...
typedef struct avb1722_Talker_StreamConfig_t {
    ...
    uint8_t destMACAdrs[MAC_ADRS_BYTE_COUNT];
    //! the source mac address - typically my own address
    uint8_t srcMACAdrs[MAC_ADRS_BYTE_COUNT];
    //! the stream ID
    ...
    //! From 61883: "A data block contains all data arriving at the transmitter
    //! within an audio sample period. The data block contains all the data
    //! which make up an event
    int dbc_at_start_of_last_packet;
    //! Number of samples per packet in the audio fifo (known as the
    //! SYT_INTERVAL in 61883)
 ...
 */
void avb1722_set_buffer_vlan(int vlan, uint8_t Buf[]);
 ...
 */
void AVB1722_Talker_bufInit(uint8_t Buf[],
                            REFERENCE_PARAM(avb1722_Talker_StreamConfig_t, pStreamConfig),
                            int vlan_id);
...
extern "C" {
#endif
int avb1722_create_packet(uint8_t Buf[],
                          REFERENCE_PARAM(avb1722_Talker_StreamConfig_t, stream_info),
...
#define MAX_PKT_BUF_SIZE_TALKER                                                                    \
    (AVB_ETHERNET_HDR_SIZE + AVB_TP_HDR_SIZE + AVB_CIP_HDR_SIZE +                                  \
     AVB1722_TALKER_MAX_NUM_SAMPLES_PER_CHANNEL * AVB_MAX_CHANNELS_PER_TALKER_STREAM * 4 + 4)
#endif

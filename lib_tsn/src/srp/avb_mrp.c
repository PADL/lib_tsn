...
#define MAX_MRP_MSG_SIZE                                                                           \
    (sizeof(mrp_msg_header) + sizeof(srp_talker_first_value) + 1 /* for event vector */ +          \
     sizeof(mrp_msg_footer))
...
uint8_t mvrp_dest_mac[6] = AVB_MVRP_MACADDR;
uint8_t srp_dest_mac[6] = AVB_SRP_MACADDR;
//!@}
...
//! need to be sorted so that they can be merged into vectors in the MRP
//! messages
...
void mrp_store_ethernet_interface(CLIENT_INTERFACE(ethernet_tx_if, i)) { i_eth = i; }
...
void debug_print_applicant_state_change(mrp_attribute_state *st, mrp_event event, int new) {
    if ((st)->attribute_type == MSRP_TALKER_ADVERTISE || (st)->attribute_type == MSRP_LISTENER ||
        ...
        debug_printf(
            "AP: %s %x:%d:%d:%d\t %s: %s -> %s \n", debug_attribute_type[(st)->attribute_type],
            stream_id[1], st->port_num, st->here, st->propagated, debug_mrp_event[(event)],
            debug_mrp_applicant_state[(st)->applicant_state], debug_mrp_applicant_state[new]);
    }
}
...
void debug_print_registrar_state_change(mrp_attribute_state *st, mrp_event event, int new) {
    if ((st)->attribute_type == MSRP_TALKER_ADVERTISE || (st)->attribute_type == MSRP_LISTENER) {
        ...
        debug_printf(
            "RG: %s %x:%d:%d:%d\t %s: %s -> %s \n", debug_attribute_type[(st)->attribute_type],
            stream_id[1], st->port_num, st->here, st->propagated, debug_mrp_event[(event)],
            debug_mrp_registrar_state[(st)->registrar_state], debug_mrp_registrar_state[new]);
    }
}
...
void debug_print_tx_event(mrp_attribute_state *st, mrp_attribute_event event) {
    if ((st)->attribute_type == MSRP_TALKER_ADVERTISE || (st)->attribute_type == MSRP_LISTENER) {
        ...
        debug_printf("TX: %s %x:%d:%d:%d\t %s \n", debug_attribute_type[(st)->attribute_type],
                     stream_id[1], st->port_num, st->here, st->propagated,
                     debug_attribute_event[(event)]);
    }
}

static void configure_send_buffer(uint8_t *addr, short etype) {
...
static void create_empty_msg(mrp_attribute_type attr, int leave_all) {
    ...
    int vector_length = 0;
    int first_value_length = first_value_lengths[attr];
    int attr_list_length =
        first_value_length + sizeof(mrp_vector_header) + vector_length + sizeof(mrp_footer);
    ...
    if (MRP_DEBUG_ATTR_EGRESS) {
        if ((st)->attribute_type == MSRP_TALKER_ADVERTISE ||
            ...
            debug_printf("TX: %s %s, stream %x:%x\n", debug_attribute_type[(st)->attribute_type],
                         debug_attribute_event[(vector)], stream_id[0], stream_id[1]);
        }
    }
...
static void mrp_update_state(mrp_event e,
    ...
    case MRP_EVENT_REDECLARE:
        if (e == MRP_EVENT_RECEIVE_LEAVE_ALL) {
            if (st->attribute_type == MVRP_VID_VECTOR) {
                start_avb_timer(&mvrp_leaveall_timer[port_num],
                                MRP_LEAVEALL_TIMER_PERIOD_CENTISECONDS /
                                    MRP_LEAVEALL_TIMER_MULTIPLIER);
                mvrp_leaveall_active[port_num] = 0;
            } else {
                start_avb_timer(&msrp_leaveall_timer[port_num],
                                MRP_LEAVEALL_TIMER_PERIOD_CENTISECONDS /
                                    MRP_LEAVEALL_TIMER_MULTIPLIER);
...
void mrp_mad_join(mrp_attribute_state *st, int new) {
    ...
    if (st->attribute_type == MSRP_LISTENER)
        debug_printf("Listener MAD_Join\n");
    else if (st->attribute_type == MSRP_TALKER_ADVERTISE)
        debug_printf("Talker MAD_Join\n");
    ...
    if (st->attribute_type == MSRP_LISTENER || st->attribute_type == MSRP_TALKER_ADVERTISE) {
        ...
        debug_printf(" %x:%x, Port:%d, Here:%d, propagated:%d\n", stream_id[0], stream_id[1],
                     st->port_num, st->here, st->propagated);
    }
#endif
...
void mrp_mad_leave(mrp_attribute_state *st) {
    if (st->attribute_type == MSRP_LISTENER)
        debug_printf("Listener MAD_Leave\n");
    else if (st->attribute_type == MSRP_TALKER_ADVERTISE)
        debug_printf("Talker MAD_Leave\n");
...
void mrp_init(char *macaddr) {
    ...
    for (int i = 0; i < MRP_NUM_PORTS; i++) {
        ...
        start_avb_timer(&periodic_timer[i],
                        MRP_PERIODIC_TIMER_PERIOD_CENTISECONDS / MRP_PERIODIC_TIMER_MULTIPLIER);
        ...
        start_avb_timer(&msrp_leaveall_timer[i],
                        MRP_LEAVEALL_TIMER_PERIOD_CENTISECONDS / MRP_LEAVEALL_TIMER_MULTIPLIER);
        init_avb_timer(&mvrp_leaveall_timer[i], MRP_LEAVEALL_TIMER_MULTIPLIER);
        start_avb_timer(&mvrp_leaveall_timer[i],
                        MRP_LEAVEALL_TIMER_PERIOD_CENTISECONDS / MRP_LEAVEALL_TIMER_MULTIPLIER);
...
static int compare_attr(mrp_attribute_state *a, mrp_attribute_state *b) {
    ...
    while (attr != NULL) {
        ...
        if (compare_attr(attr, ins)) {
            ...
            while (ins != NULL) {
                ...
                if (ins_next == NULL || (compare_attr(ins, attr) && compare_attr(attr, ins_next))) {
                    attr->next = ins->next;
                    ins->next = attr;
                    ins_next = NULL;
                }
...
static void global_event(mrp_event e, unsigned int port_num) {
    ...
    while (attr != NULL) {
        if (attr->applicant_state != MRP_DISABLED && attr->applicant_state != MRP_UNUSED &&
...
static void attribute_type_event(mrp_attribute_type atype, mrp_event e, unsigned int port_num) {
    ...
    while (attr != NULL) {
        if (attr->applicant_state != MRP_DISABLED && attr->applicant_state != MRP_UNUSED &&
            attr->attribute_type == atype && attr->port_num == port_num) {
...
static void send_join_indication(CLIENT_INTERFACE(avb_interface, avb),
                                 mrp_attribute_state *st,
                                 int new,
                                 int four_packed_event) {
    switch (st->attribute_type) {
...
static void send_leave_indication(CLIENT_INTERFACE(avb_interface, avb),
                                  mrp_attribute_state *st,
                                  int four_packed_event) {
    switch (st->attribute_type) {
    case MSRP_TALKER_ADVERTISE:
    ...
    for (int i = 0; i < MRP_NUM_PORTS; i++) {
        ...
        if (avb_timer_expired(&joinTimer[i])) {
            ...
            if (mvrp_leaveall_active[i]) {
                create_empty_msg(MVRP_VID_VECTOR, 1);
                send(i_eth, i);
                mvrp_leaveall_active[i] = 0;
            }
            ...
            if (msrp_leaveall_active[i]) {
                create_empty_msg(MSRP_TALKER_ADVERTISE, 1);
                send(i_eth, i);
                create_empty_msg(MSRP_TALKER_FAILED, 1);
                send(i_eth, i);
                create_empty_msg(MSRP_LISTENER, 1);
                send(i_eth, i);
                create_empty_msg(MSRP_DOMAIN_VECTOR, 1);
                send(i_eth, i);
                msrp_leaveall_active[i] = 0;
            }
        ...
        for (int j = 0; j < MRP_MAX_ATTRS; j++) {
            ...
            // that are declaring.
            if (attrs[j].applicant_state == MRP_VP || attrs[j].applicant_state == MRP_VN ||
                attrs[j].applicant_state == MRP_AN ||
                attrs[j].applicant_state == MRP_AA | attrs[j].applicant_state == MRP_QA ||
                attrs[j].applicant_state == MRP_LA || attrs[j].applicant_state == MRP_AP) {
                if ((attrs[j].attribute_type == MSRP_TALKER_ADVERTISE) &&
                    ...
                    if (attrs[j].here)
                        mrp_mad_join(&attrs[j], 1);
                           ...
                           !srp_domain_boundary_port[i] && reservation &&
                           reservation->failure_code == 8) {
...
static void mrp_in(int three_packed_event,
                   int four_packed_event,
                   mrp_attribute_state *st,
                   unsigned int port_num) {
...
mrp_match_type_non_prop_attribute(int attr_type, unsigned stream_id[2], int port_num) {
    for (int j = 0; j < MRP_MAX_ATTRS; j++) {
        ...
        if (attr_type == attrs[j].attribute_type && !attrs[j].propagated &&
...
mrp_attribute_state *mrp_match_attr_by_stream_and_type(mrp_attribute_state *attr,
                                                       int opposite_port,
                                                       int match_disabled) {
    for (int j = 0; j < MRP_MAX_ATTRS; j++) {
        if (attrs[j].applicant_state == MRP_UNUSED ||
            (!match_disabled && attrs[j].applicant_state == MRP_DISABLED)) {
            continue;
        }
        if ((opposite_port && (attr->port_num != attrs[j].port_num)) ||
            ...
            if ((attr->attribute_type == attrs[j].attribute_type) ||
                ...
                if (sink_info == NULL || source_info == NULL)
                    continue;
...
mrp_attribute_state *mrp_match_attribute_pair_by_stream_id(mrp_attribute_state *attr,
    ...
    for (int j = 0; j < MRP_MAX_ATTRS; j++) {
        ...
        if ((opposite_port && (attr->port_num != attrs[j].port_num)) ||
            (!opposite_port && (attr->port_num == attrs[j].port_num))) {
            if (((attr->attribute_type == MSRP_TALKER_ADVERTISE ||
                  attr->attribute_type == MSRP_TALKER_FAILED) &&
...
static int match_attribute_of_same_type(mrp_attribute_type attr_type,
                                        ...
                                        unsigned int port_num,
                                        int leave_all) {
    if (attr->applicant_state == MRP_UNUSED || attr->applicant_state == MRP_DISABLED)
        return 0;
    ...
    if ((attr->attribute_type <= MSRP_TALKER_FAILED) && (attr_type <= MSRP_TALKER_FAILED)) {
        // Continue, match Talker Advertise and Failed as the same attribute
        // type
...
void avb_mrp_process_packet(uint8_t *buf, int etype, int len, unsigned int port_num) {
    ...
    while (msg < end && (msg[0] != 0 || msg[1] != 0)) {
        ...
        while (msg < end && (msg[0] != 0 || msg[1] != 0)) {
            ...
            int len =
                sizeof(mrp_vector_header) + first_value_len + threepacked_len + fourpacked_len;

            if ((etype == AVB_SRP_ETHERTYPE) &&
                (len + sizeof(mrp_msg_footer) > attribute_list_length(hdr))) {
                return;
            }
            ...
            for (int i = 0; i < numvalues; i++) {
                ...
                if (vector > 0xD7)
                    break; // Unused range of the threepacked vector should be
                ...
                int four_packed_event =
                    has_fourpacked_events(attr_type)
                        ? decode_fourpacked(
                              *(first_value + first_value_len + threepacked_len + i / 4), i % 4)
                        : 0;
                ...
                // This allows the application state machines to respond to the
                // message
                for (int j = 0; j < MRP_MAX_ATTRS; j++) {
                    // Attempt to match to this endpoint's attributes
                    if (match_attribute_of_same_type(attr_type, &attrs[j], first_value, i,
                ...
                if (MRP_NUM_PORTS == 2) {
                    if (!matched_attribute && !leave_all) {
                        if (attr_type == MSRP_TALKER_ADVERTISE || attr_type == MSRP_LISTENER) {
                            if (three_packed_event != MRP_ATTRIBUTE_EVENT_MT) {
                                mrp_attribute_state *st = avb_srp_process_new_attribute_from_packet(
                                    attr_type, first_value, i, port_num);
                                if (st) {
                                    mrp_mad_begin(st);

...
/* This needs to be greater than the actual max number of handled streams,
   because SRP cannot remove the attributes as quickly as a connection can be
   torn down and setup again on the Mac. Hence there will be a period where the
   number of active stream entries is greater than the max capable number until
   they are cleaned up.
*/
...
void srp_store_ethernet_interface(CLIENT_INTERFACE(ethernet_if, i)) { i_eth = i; }
...
static int srp_calculate_stream_bandwidth(int max_frame_size, int extra_byte) {
    ...
    const int crc = 4;
    const int total_frame_size =
        interframe_gap + preamble_sfd + eth_header_and_tag + max_frame_size + crc + extra_byte;
...
int avb_srp_match_listener_to_talker_stream_id(unsigned stream_id[2],
...
// Either return an index to update, or a new index if not matched, or -1 if no
// entries free
...
avb_stream_entry *srp_add_reservation_entry_stream_id_only(unsigned int stream_id[2]) {
    ...
    if (entry >= 0) {
        if (!stream_table[entry].talker_present)
            memset(&stream_table[entry].reservation, 0, sizeof(avb_srp_info_t));
...
avb_stream_entry *srp_add_reservation_entry(avb_srp_info_t *reservation) {
    ...
    if (entry >= 0) {
        const int reservation_size_minus_failure_info =
            sizeof(avb_srp_info_t) -
            (sizeof(avb_srp_info_t) - offsetof(avb_srp_info_t, failure_bridge_id));
        memcpy(&stream_table[entry].reservation, reservation, reservation_size_minus_failure_info);
        debug_printf("Added stream:\n ID: %x%x\n DA:", reservation->stream_id[0],
                     reservation->stream_id[1]);
        for (int i = 0; i < 6; i++) {
            printhex(stream_table[entry].reservation.dest_mac_addr[i]);
            printchar(':');
        }
        debug_printf("\n max size: %d\n interval: %d\n",
...
void srp_remove_reservation_entry(avb_srp_info_t *reservation) {
    ...
    if (entry >= 0) {
        debug_printf("Removed stream:\n ID: %x%x\n", reservation->stream_id[0],
                     reservation->stream_id[1]);
        memset(&stream_table[entry], 0x00, sizeof(avb_stream_entry));
    } else {
        debug_printf("Assert: Tried to remove a reservation that isn't stored: %x%d",
...
int srp_cleanup_reservation_entry(mrp_event event, mrp_attribute_state *st) {
    ...
    if (st->attribute_type == MSRP_TALKER_ADVERTISE || st->attribute_type == MSRP_TALKER_FAILED ||
        ...
        // If there is no match, it is either because there is genuinely no
        // attribute match or if the reservation entry has been zeroed Hence the
        // stream ID is zero
...
// TODO: Remove listener bool param and just use the attribute type from the
// state structure
static void avb_srp_map_join(mrp_attribute_state *attr, int new, int listener) {
...
#endif
    mrp_attribute_state *matched_talker_listener =
        mrp_match_attribute_pair_by_stream_id(attr, 1, 0);
    mrp_attribute_state *matched_stream_id_opposite_port =
        mrp_match_attr_by_stream_and_type(attr, 1, 0);
  ...
  debug_printf("matched_talker_listener: %d(here:%d, prop:%d, new:%d), matched_stream_id_opposite_port: %d\n", matched_talker_listener,
   matched_talker_listener ? matched_talker_listener->propagated : 0, matched_talker_listener ? matched_talker_listener->here : 0, new, matched_stream_id_opposite_port);
    ...
    } else if (!matched_stream_id_opposite_port && matched_talker_listener &&
               !matched_talker_listener->propagated && !matched_talker_listener->here) {
        create_propagated_attribute_and_join(attr, new);
    }
        ...
        if (!matched_talker_listener->here) { // Handle case where the Talker is not this endpoint
            ...
            if (!matched_talker_listener->here) {
                ...
                if (!stream_table[entry].bw_reserved[attr->port_num]) {
                    ...
                    srp_increase_port_bandwidth(attribute_info->tspec_max_frame_size, 1,
                                                attr->port_num);
    ...
    if (attr->attribute_type == MSRP_LISTENER) {
        ...
        if (matched_stream_id_opposite_port) {
            if (matched_talker_listener &&
                !matched_talker_listener->here) { // We are not the Talker
                if (stream_table[entry].bw_reserved[attr->port_num]) {
                    srp_decrease_port_bandwidth(attribute_info->tspec_max_frame_size, 1,
                                                attr->port_num);
                    avb_1722_disable_stream_forwarding(i_eth, attribute_info->stream_id);
                    stream_table[entry].bw_reserved[attr->port_num] = 0;
                    // Propagate Listener leave only if we are not also
                    // Listening to this stream
                    if (matched_stream_id_opposite_port->propagated &&
                        !matched_stream_id_opposite_port->here) {
        ...
        if (matched_talker_listener &&
            stream_table[entry].bw_reserved[matched_talker_listener->port_num]) {
            srp_decrease_port_bandwidth(attribute_info->tspec_max_frame_size, 1,
                                        matched_talker_listener->port_num);
            avb_1722_disable_stream_forwarding(i_eth, attribute_info->stream_id);
        ...
        mrp_attribute_state *matched_listener_this_port =
            mrp_match_attribute_pair_by_stream_id(attr, 0, 0);

        if (matched_listener_this_port) {
            /* Special case of behaviour described in 25.3.4.4.1.
             * "the Bridge shall act as a proxy for the Listener(s) and
             * automatically generate a MAD_Leave.request back toward the Talker
             * for those Listener attributes"
             */
...
int avb_srp_match_talker_advertise(
    ...
    uint8_t sr_class_priority = (first_value->TSpec >> 5) & 7;
    ...
    if (!leave_all && (my_stream_id == stream_id)) {
        ...
        if (failed) {
            ...
            debug_printf("WARNING: Talker failed (Stream ID: %x%x, failure code: %d)\n",
                         ...
                         source_info->reservation.stream_id[1], first_value->FailureCode);
...
int avb_srp_match_domain(mrp_attribute_state *attr, char *fv, int i) {
    ...
    uint16_t sr_class_vid = ntoh_16(first_value->SRclassVID);

    if ((sr_class_id == AVB_SRP_SRCLASS_DEFAULT) &&
        (sr_class_priority == AVB_SRP_TSPEC_PRIORITY_DEFAULT)) {
...
void avb_srp_listener_join_ind(CLIENT_INTERFACE(avb_interface, avb),
...
#if (MRP_NUM_PORTS == 2)
        if (mrp_match_attr_by_stream_and_type(attr, 1, 0)) { // Listener ready on the other port
                                                             // also, therefore send on both ports
            if (stream_table[entry].bw_reserved[!attr->port_num] == 1 &&
                stream_table[entry].bw_reserved[attr->port_num] != 1) {
                srp_increase_port_bandwidth(sink_info->reservation.tspec_max_frame_size, 0,
                                            attr->port_num);
...
#endif
            if (mrp_match_type_non_prop_attribute(MSRP_TALKER_ADVERTISE,
                                                  sink_info->reservation.stream_id,
                                                  attr->port_num)) { // Just this port
            if (stream_table[entry].bw_reserved[attr->port_num] != 1) {
                srp_increase_port_bandwidth(sink_info->reservation.tspec_max_frame_size, 0,
                                            attr->port_num);
...
void avb_srp_listener_leave_ind(CLIENT_INTERFACE(avb_interface, avb),
                                mrp_attribute_state *attr,
                                int four_packed_event) {
    enum avb_source_state_t state;
    avb_sink_info_t *sink_info = (avb_sink_info_t *)attr->attribute_info;
    unsigned stream = avb_get_source_stream_index_from_stream_id(sink_info->reservation.stream_id);
    mrp_attribute_state *matched_listener_opposite_port =
        mrp_match_attr_by_stream_and_type(attr, 1, 0);
    ...
    if (stream != -1u) {
        if (stream_table[entry].bw_reserved[attr->port_num] == 1) {
            srp_decrease_port_bandwidth(sink_info->reservation.tspec_max_frame_size, 0,
                                        attr->port_num);
...
void avb_srp_leave_talker_attrs(unsigned int stream_id[2]) {
    for (int i = 0; i < MRP_NUM_PORTS; i++) {
        mrp_attribute_state *matched_talker =
            mrp_match_type_non_prop_attribute(MSRP_TALKER_ADVERTISE, stream_id, i);
        if (!matched_talker)
            matched_talker = mrp_match_type_non_prop_attribute(MSRP_TALKER_FAILED, stream_id, i);

        if (matched_talker) {
            mrp_mad_leave(matched_talker);
        }
#if (MRP_NUM_PORTS == 1)
        mrp_attribute_state *matched_listener =
            mrp_match_type_non_prop_attribute(MSRP_LISTENER, stream_id, 0);
...
short avb_srp_create_and_join_talker_advertise_attrs(avb_srp_info_t *reservation) {
    ...
    if (stream_ptr) {
        for (int i = 0; i < MRP_NUM_PORTS; i++) {
            mrp_attribute_state *matched_talker =
                mrp_match_type_non_prop_attribute(MSRP_TALKER_ADVERTISE, reservation->stream_id, i);
            if (!matched_talker)
                matched_talker = mrp_match_type_non_prop_attribute(MSRP_TALKER_FAILED,
                                                                   reservation->stream_id, i);

            if (reservation->vlan_id == 0) {
                // A VID of 0 indicates that we should join the VID from the
                // domain
...
#if MRP_NUM_PORTS == 1
            mrp_attribute_state *matched_listener =
                mrp_match_type_non_prop_attribute(MSRP_LISTENER, reservation->stream_id, i);
            if (!matched_listener) {
...
void avb_srp_leave_listener_attrs(unsigned int stream_id[2]) {
    ...
    mrp_attribute_state *matched_talker =
        mrp_match_type_non_prop_attribute(MSRP_TALKER_ADVERTISE, stream_id, -1);
    if (!matched_talker)
        matched_talker = mrp_match_type_non_prop_attribute(MSRP_TALKER_FAILED, stream_id, -1);

    if (matched_talker) {
        mrp_attribute_state *matched_listener_opposite_port =
            mrp_match_attribute_pair_by_stream_id(matched_talker, 1, 0);
        mrp_attribute_state *matched_listener_this_port =
            mrp_match_attribute_pair_by_stream_id(matched_talker, 0, 0);

        // LL2: If there is a non-propagated Listener attr on the opposite port,
        // then there is another Listener Do not leave
        if (matched_listener_opposite_port) {
            if (matched_listener_this_port) {
                // Reset the here flag so that the Lv is propagated when a
                // Listener down the chain leaves - bug 15186
    ...
    mrp_attribute_state *matched_listener =
        mrp_match_type_non_prop_attribute(MSRP_LISTENER, stream_id, 0);
    mrp_attribute_state *matched_talker =
        mrp_match_type_non_prop_attribute(MSRP_TALKER_ADVERTISE, stream_id, 0);
    if (!matched_talker)
        matched_talker = mrp_match_type_non_prop_attribute(MSRP_TALKER_FAILED, stream_id, 0);
...
short avb_srp_join_listener_attrs(unsigned int stream_id[2], short vlan_id) {
    ...
    mrp_attribute_state *matched_talker_advertise =
        mrp_match_type_non_prop_attribute(MSRP_TALKER_ADVERTISE, stream_id, -1);
    mrp_attribute_state *matched_talker_failed =
        mrp_match_type_non_prop_attribute(MSRP_TALKER_FAILED, stream_id, -1);

    short vid_to_join = vlan_id == 0 ? current_vlan_id_from_domain : vlan_id;

    if (matched_talker_advertise) {
        mrp_attribute_state *matched_listener_same_port =
            mrp_match_attribute_pair_by_stream_id(matched_talker_advertise, 0, 0);
        ...
        if (!matched_listener_same_port) {
            ...
            if (matched_listener_same_port) {
                ...
                mrp_attribute_init(matched_listener_same_port, MSRP_LISTENER,
                                   matched_talker_advertise->port_num, 1,
...
#if (MRP_NUM_PORTS == 2)
            // If we are attaching to a stream already being Listener to down
            // stream, mark it as originating here so that we do not propagate a
            // Leave when we disconnect
            matched_listener_same_port->here = 1;
#endif
            mrp_mad_join(matched_listener_same_port, 1);
        }
    } else if (!matched_talker_failed) { // LJ4: If the Talker advertise hasn't
                                         // matched, then it probably hasn't
                                         // arrived yet
        ...
        if (stream_ptr) {
            for (int i = 0; i < MRP_NUM_PORTS; i++) {
                mrp_attribute_state *matched_listener =
                    mrp_match_type_non_prop_attribute(MSRP_LISTENER, stream_id, i);
                ...
                if (!matched_talker_advertise) {
                    ...
                    mrp_attribute_state *talker_attr = mrp_get_attr();
                    if (talker_attr) {
                        mrp_attribute_init(talker_attr, MSRP_TALKER_ADVERTISE, i, 0, stream_ptr);
                        mrp_mad_begin(talker_attr);
                    }
                }
#endif
...
void avb_srp_talker_join_ind(mrp_attribute_state *attr, int new) {
    if (MRP_NUM_PORTS == 2) {
        avb_sink_info_t *sink_info = (avb_sink_info_t *)attr->attribute_info;
        unsigned stream =
            avb_get_sink_stream_index_from_stream_id(sink_info->reservation.stream_id);
        mrp_attribute_state *matched_listener_this_port =
            mrp_match_attribute_pair_by_stream_id(attr, 0, 1);

        /* This covers the case where the Listener joins before the Talker
         * attribute. When we receive the Talker join new, we also trigger join
         * for the Listener attribute on the same port if it already exists. We
         * also need to mark the disabled Listener attr we created on the other
         * port as unused
         */
        if (stream != -1u && matched_listener_this_port && matched_listener_this_port->here) {
            mrp_attribute_state *matched_listener_opposite_port =
                mrp_match_attribute_pair_by_stream_id(attr, 1, 1);
            if (matched_listener_opposite_port) {
                mrp_change_applicant_state(matched_listener_opposite_port, MRP_EVENT_DUMMY,
                                           MRP_UNUSED);
            }
...
void avb_srp_talker_leave_ind(mrp_attribute_state *attr) {
    if (MRP_NUM_PORTS == 2) {
        ...
        if (stream != -1u) {
            // This could be used to report talker advertising instead of the
            // snooping scheme above
        } else {
            avb_srp_map_leave(attr);
        }
    } else {
...
static int encode_listener_message(char *buf, mrp_attribute_state *st, int vector) {
    ...
    if (merge) {
        ...
        if (stream_info->talker_present && !srp_domain_boundary_port[st->port_num] &&
            !stream_info->reservation_failed) {
            mrp_encode_four_packed_event(buf, AVB_SRP_FOUR_PACKED_EVENT_READY, st->attribute_type);
        } else {
            mrp_encode_four_packed_event(buf, AVB_SRP_FOUR_PACKED_EVENT_ASKING_FAILED,
                                         st->attribute_type);
        }
...
void avb_srp_domain_join_ind(CLIENT_INTERFACE(avb_interface, avb),
                             mrp_attribute_state *attr,
                             int new) {
    debug_printf("Joined SRP domain (VID %x, port %d)\n", current_vlan_id_from_domain,
                 attr->port_num);
...
static int check_talker_firstvalue_merge(char *buf, avb_source_info_t *source_info) {
    ...
    int num_values = hdr->NumberOfValuesLow;
    uint64_t stream_id = 0, my_stream_id = 0;
    uint64_t dest_addr = 0, my_dest_addr = 0;
...
static int encode_talker_message(char *buf, mrp_attribute_state *st, int vector) {
    ...
    if ((st->attribute_type == MSRP_TALKER_ADVERTISE) &&
        (mrp_hdr->AttributeType != AVB_SRP_ATTRIBUTE_TYPE_TALKER_ADVERTISE)) {
        return 0;
    }

    if ((st->attribute_type == MSRP_TALKER_FAILED) &&
        (mrp_hdr->AttributeType != AVB_SRP_ATTRIBUTE_TYPE_TALKER_FAILED)) {
        return 0;
    }
    ...
    if (merge) {
        ...
        if (num_values == 0) {
            ...
            hton_16(first_value->TSpecMaxIntervalFrames, attribute_info->tspec_max_interval);
            hton_32(first_value->AccumulatedLatency, attribute_info->accumulated_latency);

            if (st->attribute_type == MSRP_TALKER_FAILED) {
                srp_talker_failed_first_value *first_value =
                    (srp_talker_failed_first_value *)(buf + sizeof(mrp_msg_header) +
...
int avb_srp_compare_talker_attributes(mrp_attribute_state *a, mrp_attribute_state *b) {

...
void avb_process_srp_control_packet(client interface avb_interface avb,
                                    unsigned int buf0[],
                                    unsigned nbytes,
    ...
    if (packet_type == ETH_IF_STATUS) {
        ...
        if (has_qtag) {
            ...
            etype = (int)(ethernet_hdr->ethertype.data[0] << 8) +
                    (int)(ethernet_hdr->ethertype.data[1]);
        }
...
[[combinable]] void avb_srp_task(client interface avb_interface i_avb,

...
#define AVB_SRP_MACADDR                                                                            \
    { 0x01, 0x80, 0xc2, 0x00, 0x00, 0xe }

typedef struct avb_stream_entry {
    avb_srp_info_t reservation;
    char listener_present;
    char talker_present;
    char bw_reserved[MRP_NUM_PORTS]; // While the bw_reserved flag is set/not
                                     // set we do not add/subtract Qav credit

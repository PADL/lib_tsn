...
typedef enum { MRP_IN, MRP_LV, MRP_MT } mrp_registrar_state;
...
typedef struct mrp_attribute_state {
    ...
    uint8_t remove_after_next_tx;
#ifdef MRP_FULL_PARTICIPANT
    uint8_t registrar_state;
    avb_timer leaveTimer;
#endif

    //! used to note indications that have been detected by the state machine
    //! but not indicated to the application yet
    short pending_indications;

    //! if there is a pending indication and it had an associated four vector
    //! parameter then the parameter is stored here
    short four_vector_parameter;

    //! While sorting the attributes, this contains a linked list of sorted
    //! attributes
    ...
    //! Generic pointer to allow random data to be stored alongside the
    //! attribute
    void *attribute_info;
} mrp_attribute_state;
#ifdef __XC__

...
enum avb_stream_format_t {
    ...
    AVB_FORMAT_AAF_32BIT,  /*< 32bit AAF */
    AVB_FORMAT_AAF_24BIT,  /*< 24bit AAF */
};

/** The state of an AVB source (Talker). */
enum avb_source_state_t {
    AVB_SOURCE_STATE_DISABLED,  /*!< The source is disabled and will not
                                   transmit. */
...
enum avb_sink_state_t {
    ...
    AVB_SINK_STATE_POTENTIAL, /*!< The sink is enabled and will pass audio when
                                 a Talker requests it */
...
enum device_media_clock_type_t {
    DEVICE_MEDIA_CLOCK_INPUT_STREAM_DERIVED, /*!< The clock is sourced from the
                                                media clock of an Input Stream
                                              */
    DEVICE_MEDIA_CLOCK_LOCAL_CLOCK, /*!< The clock is sourced from within the
                                       entity from the local crystal oscillator
                                     */
    DEVICE_MEDIA_CLOCK_PTP /*!< The clock is sourced directly from the gPTP
                              clock */
};

/** A set of media related commands generated by the AVB manager */
enum device_media_clock_commands_t {
    DEVICE_MEDIA_CLOCK_SET_SAMPLING_RATE /*!< A command to indicate a change in
...
typedef struct media_clock_info_t {
    ...
    int lock_counter;   ///< A count of the number of lock events on this media
                        ///< clock
    int unlock_counter; ///< A count of the number of unlock events on this
                        ///< media clock
} media_clock_info_t;
...
typedef struct avb_srp_info_t {
    ...
    short tspec_max_interval;       /**< Maximum number of frames sent per class
                                       measurement interval */
    unsigned char tspec;            /**< Data Frame Priority and Rank fields */
    unsigned accumulated_latency;   /**< Latency at ingress port for Talker
                                     * registrations, or latency at   end of egress
                                     * media for Listener Declarations */
    unsigned char failure_bridge_id[8]; /**< Bridge ID of bridge that changed
                                           Talker Advertise to Talker Failed */
    unsigned char
        failure_code; /**< Failure code associated with the failure bridge */
} avb_srp_info_t;
...
interface avb_interface {
    /** Intended for internal use within client interface get and set extensions
     * only */
...
extends client interface avb_interface : {
     ...
     */
    static inline int get_source_format(client interface avb_interface i,
                                        unsigned source_num,
     ...
     *  \param rate       the sample rate of the stream in Hz
     */
    static inline int set_source_format(client interface avb_interface i,
                                        unsigned source_num,
                                        enum avb_stream_format_t format,
                                        int rate) {
        if (source_num >= AVB_NUM_SOURCES)
     ...
     */
    static inline int get_source_channels(client interface avb_interface i,
                                          unsigned source_num,
                                          int &channels) {
        if (source_num >= AVB_NUM_SOURCES)
            return 0;
     ...
     *  \param channels     the number of channels
     */
    static inline int set_source_channels(client interface avb_interface i,
                                          unsigned source_num,
                                          int channels) {
        if (source_num >= AVB_NUM_SOURCES)
            return 0;
     ...
     */
    static inline int get_source_sync(client interface avb_interface i,
                                      unsigned source_num,
                                      int &sync) {
        if (source_num >= AVB_NUM_SOURCES)
            return 0;
     ...
     *  \param sync         the media clock number
     */
    static inline int set_source_sync(client interface avb_interface i,
                                      unsigned source_num,
                                      int sync) {
        if (source_num >= AVB_NUM_SOURCES)
            return 0;
     ...
     */
    static inline int get_source_presentation(client interface avb_interface i,
                                              unsigned source_num,
                                              int &presentation) {
        if (source_num >= AVB_NUM_SOURCES)
            return 0;
     ...
     *
     **/
    static inline int set_source_presentation(client interface avb_interface i,
                                              unsigned source_num,
                                              int presentation) {
        if (source_num >= AVB_NUM_SOURCES)
            return 0;
     ...
     */
    static inline int get_source_vlan(client interface avb_interface i,
                                      unsigned source_num,
                                      int &vlan) {
        if (source_num >= AVB_NUM_SOURCES)
            return 0;
     ...
     *  \param vlan       the destination vlan id
     */
    static inline int set_source_vlan(client interface avb_interface i,
                                      unsigned source_num,
                                      int vlan) {
        if (source_num >= AVB_NUM_SOURCES)
            return 0;
     ...
     */
    static inline int get_source_state(client interface avb_interface i,
                                       unsigned source_num,
     ...
     *  state to ``ENABLED``. Changing the state to
     * ``AVB_SOURCE_STATE_POTENTIAL`` turns the stream on and SRP will
     * automatically set it to ``ENABLED`` when connected to a listener and
     * streaming.
     *
     *  \param i          interface to AVB manager
     *  \param source_num the local source number
     *  \param state      the state of the source
     */
    static inline int set_source_state(client interface avb_interface i,
                                       unsigned source_num,
                                       enum avb_source_state_t state) {
        if (source_num >= AVB_NUM_SOURCES)
            return 0;
     ...
     *             make up the stream
     *  \param len the length of the map; should be equal to the number of
     * channels in the stream
     */
    static inline int get_source_map(client interface avb_interface i,
                                     unsigned source_num,
                                     int map[],
                                     int &len) {
        if (source_num >= AVB_NUM_SOURCES)
     ...
     *
     **/
    static inline int set_source_map(client interface avb_interface i,
                                     unsigned source_num,
                                     int map[len],
                                     unsigned len) {
        if (source_num >= AVB_NUM_SOURCES)
     ...
     *  \param len          the length of the address, should always be equal to
     * 6
     */
    static inline int get_source_dest(client interface avb_interface i,
                                      unsigned source_num,
                                      unsigned char addr[],
                                      int &len) {
        if (source_num >= AVB_NUM_SOURCES)
     ...
     *
     **/
    static inline int set_source_dest(client interface avb_interface i,
                                      unsigned source_num,
    ...
    static inline int get_source_id(client interface avb_interface i,
                                    unsigned source_num,
                                    unsigned int id[2]) {
        if (source_num >= AVB_NUM_SOURCES)
            return 0;
     ...
     */
    static inline int get_sink_id(client interface avb_interface i,
                                  unsigned sink_num,
     ...
     *
     */
    static inline int set_sink_id(client interface avb_interface i,
                                  unsigned sink_num,
                                  unsigned int stream_id[2]) {
        if (sink_num >= AVB_NUM_SINKS)
            return 0;
     ...
     *  \param rate       the sample rate of the stream in Hz
     */
    static inline int get_sink_format(client interface avb_interface i,
                                      unsigned sink_num,
                                      enum avb_stream_format_t &format,
                                      int &rate) {
        if (sink_num >= AVB_NUM_SINKS)
     ...
     *  \param rate         the sample rate of the stream in Hz
     */
    static inline int set_sink_format(client interface avb_interface i,
                                      unsigned sink_num,
                                      enum avb_stream_format_t format,
                                      int rate) {
        if (sink_num >= AVB_NUM_SINKS)
     ...
     *  \param channels     the number of channels
     */
    static inline int get_sink_channels(client interface avb_interface i,
                                        unsigned sink_num,
                                        int &channels) {
        if (sink_num >= AVB_NUM_SINKS)
            return 0;
     ...
     *  \param channels     the number of channels
     */
    static inline int set_sink_channels(client interface avb_interface i,
                                        unsigned sink_num,
                                        int channels) {
        if (sink_num >= AVB_NUM_SINKS)
            return 0;
     ...
     *  \param sync         the media clock number
     */
    static inline int get_sink_sync(client interface avb_interface i,
                                    unsigned sink_num,
                                    int &sync) {
        if (sink_num >= AVB_NUM_SINKS)
            return 0;
     ...
     *  \param sync         the media clock number
     */
    static inline int set_sink_sync(client interface avb_interface i,
                                    unsigned sink_num,
                                    int sync) {
        if (sink_num >= AVB_NUM_SINKS)
            return 0;
     ...
     */
    static inline int get_sink_vlan(client interface avb_interface i,
                                    unsigned sink_num,
                                    int &vlan) {
        if (sink_num >= AVB_NUM_SINKS)
            return 0;
     ...
     *
     */
    static inline int set_sink_vlan(client interface avb_interface i,
                                    unsigned sink_num,
                                    int vlan) {
        if (sink_num >= AVB_NUM_SINKS)
            return 0;
     ...
     *  \param len          The length of the address, should always be equal
     * to 6.
     */
    static inline int get_sink_addr(client interface avb_interface i,
                                    unsigned sink_num,
                                    unsigned char addr[],
                                    int &len) {
        if (sink_num >= AVB_NUM_SINKS)
     ...
     *
     **/
    static inline int set_sink_addr(client interface avb_interface i,
                                    unsigned sink_num,
                                    unsigned char addr[len],
                                    unsigned len) {
        if (sink_num >= AVB_NUM_SINKS)
     ...
     */
    static inline int get_sink_state(client interface avb_interface i,
                                     unsigned sink_num,
     ...
     *  state to ``ENABLED``. Changing the state to ``POTENTIAL`` turns the
     * stream on and it will automatically pass audio once audio is available.
     *
     * \param i     interface to AVB manager
     * \param sink_num the number of the sink
     * \param state the state of the sink
     *
     */
    static inline int set_sink_state(client interface avb_interface i,
                                     unsigned sink_num,
                                     enum avb_sink_state_t state) {
        if (sink_num >= AVB_NUM_SINKS)
            return 0;
     ...
     */
    static inline int get_sink_map(client interface avb_interface i,
                                   unsigned sink_num,
                                   int map[],
                                   int &len) {
        if (sink_num >= AVB_NUM_SINKS)
     ...
     *                   of channels in the stream
     */
    static inline int set_sink_map(client interface avb_interface i,
                                   unsigned sink_num,
                                   int map[len],
                                   unsigned len) {
        if (sink_num >= AVB_NUM_SINKS)
     ...
     */
    static inline int get_device_media_clock_rate(
        client interface avb_interface i, int clock_num, int &rate) {
     ...
     *
     **/
    static inline int set_device_media_clock_rate(
        client interface avb_interface i, int clock_num, int rate) {
        if (clock_num >= AVB_NUM_MEDIA_CLOCKS)
            return 0;
        media_clock_info_t info;
     ...
     */
    static inline int
    get_device_media_clock_state(client interface avb_interface i,
                                 int clock_num,
     ...
     **/
    static inline int
    set_device_media_clock_state(client interface avb_interface i,
                                 int clock_num,
                                 enum device_media_clock_state_t state) {
        if (clock_num >= AVB_NUM_MEDIA_CLOCKS)
            return 0;
     ...
     */
    static inline int get_device_media_clock_source(
        client interface avb_interface i, int clock_num, int &source) {
        if (clock_num >= AVB_NUM_MEDIA_CLOCKS)
            return 0;
        media_clock_info_t info;
        info = i._get_media_clock_info(clock_num);
     ...
     *
     **/
    static inline int set_device_media_clock_source(
        client interface avb_interface i, int clock_num, int source) {
        if (clock_num >= AVB_NUM_MEDIA_CLOCKS)
            return 0;
        media_clock_info_t info;
     ...
     */
    static inline int
    get_device_media_clock_type(client interface avb_interface i,
                                int clock_num,
     ...
     **/
    static inline int
    set_device_media_clock_type(client interface avb_interface i,
                                int clock_num,
                                enum device_media_clock_type_t clock_type) {
        if (clock_num >= AVB_NUM_MEDIA_CLOCKS)
            return 0;
     ...
     *
     **/
    static inline struct avb_debug_counters
    get_debug_counters(client interface avb_interface i) {
...
interface srp_interface {
    /** Used by a Talker application entity to issue a request to the MSRP
     * Participant to initiate the advertisement of an available Stream
     *
     *  \param stream_info Struct of type avb_srp_info_t containing parameters
     * of the stream to register
     */
    short register_stream_request(avb_srp_info_t stream_info);

    /** Used by a Talker application entity to request removal of the Talker’s
     * advertisement declaration, and thus remove the advertisement of a Stream,
     * from the network.
     *
     *  \param stream_id two int array containing the Stream ID of the stream to
     * deregister
     */
    void deregister_stream_request(unsigned stream_id[2]);

    /** Used by a Listener application entity to issue a request to attach to
     * the referenced Stream.
     *
     *  \param stream_id two int array containing the Stream ID of the stream to
     * register \param vlan_id   the VLAN ID associated with the stream. If 0
     * the current VID from the SRP domain will be used.
     */
    short register_attach_request(unsigned stream_id[2], short vlan_id);

    /** Used by a Listener application entity to remove the request to attach to
     * the referenced Stream.
     *
     *  \param stream_id two int array containing the Stream ID of the stream to
     * deregister
...
/** Core AVB API management task that can be combined with other AVB tasks such
 as SRP or 1722.1

 * \param i_avb[]           array of avb_interface server interfaces connected
 to clients of avb_manager
 * \param num_avb_clients   number of client interface connections to the server
 and the number of elements of i_avb[]
 * \param i_srp            client interface of type srp_interface into an
 srp_task() task
 * \param c_media_ctl[]     array of chanends connected to components that
 register/control media FIFOs
 * \param c_listener_ctl[]  array of chanends connected to components that
 register/control IEEE 1722 sinks
 * \param c_talker_ctl[]    array of chanends connected to components that
 register/control IEEE 1722 sources
 * \param i_eth_cfg         a client interface of type ethernet_cfg_if for
 Ethernet MAC configuration
 * \param i_media_clock_ctl client interface of type media_clock_if connected to
...
/** A task that runs MAAP and 1722.1 ADP, ACMP and AECP protocols and interacts
 * with the rest of the AVB stack.
 *
 ...
 *  \param  i_1722_1_entity client interface of type
 * avb_1722_1_control_callbacks \param  qspi_ports  a reference to a Quad SPI
 * flash ports structure \param  i_eth_rx a client receive interface into the
 * Ethernet MAC \param  i_eth_tx a client transmit interface into the Ethernet
 * MAC \param  i_eth_cfg a client interface for Ethernet MAC configuration
 ...
 *  \param c_eth_tx_hp      a high priority client transmit interface into the
 *Ethernet MAC \param c_talker_ctl     channel to configure the talker \param
 *num_streams      the number of streams the unit controls \param
 *audio_input_buf  a client interface to get a handle to pull from the audio
 *input buffer
 **/
 ...
 *  using the AVB control API.
 *
 *  \param c_eth_rx_hp      a high priority client receive interface into the
 * Ethernet MAC \param c_buf_ctl        buffer control link to the media clock
 * server \param c_ptp_ctl        PTP server link for retrieving PTP time info
 *  \param c_listener_ctl   channel to configure the listener (given
 *                          to avb_init())
 *  \param num_streams      the number of streams the unit will handle
 *  \param audio_output_buf a client interface to get a handle to push to the
 * audio output buffer
 */
...
/** The media clock server.
 *
 *  \param media_clock_ctl  server interface of type media_clock_if connected to
 the avb_manager() task
 ...
 *  \param i_eth_tx         a client transmit interface into the Ethernet MAC
 *  \param i_eth_cfg        a client interface for Ethernet MAC configuration
 *  \param c_ptp[]          an array of chanends to connect to clients of the
 ptp server
 *  \param num_ptp          The number of PTP clients attached
 *  \param server_type      The type of the PTP server
 (``PTP_GRANDMASTER_CAPABLE`` or ``PTP_SLAVE_ONLY``)
 */
...
int avb_get_source_state(CLIENT_INTERFACE(avb_interface, avb),
                         unsigned source_num,
                         REFERENCE_PARAM(enum avb_source_state_t, state));
int avb_set_source_state(CLIENT_INTERFACE(avb_interface, avb),
                         unsigned source_num,
                         enum avb_source_state_t state);
int avb_get_source_vlan(CLIENT_INTERFACE(avb_interface, avb),
                        unsigned source_num,
                        REFERENCE_PARAM(int, vlan));
int avb_set_source_vlan(CLIENT_INTERFACE(avb_interface, avb),
                        unsigned source_num,
                        int vlan);
int avb_get_sink_vlan(CLIENT_INTERFACE(avb_interface, avb),
                      unsigned sink_num,
                      REFERENCE_PARAM(int, vlan));
int avb_set_sink_vlan(CLIENT_INTERFACE(avb_interface, avb),
                      unsigned sink_num,
                      int vlan);
